<div class="payment-request-detail" id="print-area">
    ...
    // totals
    // Untuk show.blade.php, grandTotal masih menggunakan additional_discount untuk keperluan tampilan info SR
    // (bukan untuk kalkulasi payment, itu di PaymentShow)
    $grandTotal = $sr->details->sum('ammount') - floatval($sr->additional_discount ?? 0);
    $totalPayment = $sr->payments->sum('grand_total');
    ...
    // permissions
    $canSubmit = ($isAdmin || ($isOwner && $user->hasPermission('sr.submit'))) && in_array($status, [0, null, 12]);
    $canCancelSubmit = ($isAdmin || (($isOwner || $this->isSupervisor($sr)) && $user->hasPermission('sr.cancel_submit'))) && $status === 1;

    $canEdit = ($isAdmin || $isOwner || $user->hasPermission('sr.edit')) && in_array($status, [0, null, 12]);
    ...
    $canPayment = $isAdmin || $user->hasPermission('sr_payment.view') || $isOwner;
    $showPayment = in_array($status, [7,8,9,10,11,14,15]);
    $canDownload = ($isAdmin || $isOwner || $user->hasPermission('sr.print')) && in_array($status, [3,4,5,6,7,8,9,10,11]);

    $sign = fn(int $s) => $sr->signTransactions->where('status', $s)->first();
    ...
    11 => ['label'=>'Paid / Balance Settled', 'color'=>'success'],
    ...
    $sbadge = $statusBadge[$sr->status] ?? ['label'=>'Unknown','color'=>'dark'];
    ?>

    
    <template x-teleport="#sr-header-actions">
        <div class="d-flex align-items-center gap-2" data-html2canvas-ignore="true">
            ...
            <button type="button" class="btn btn-primary btn-sm rounded-pill shadow-sm px-3 fw-bold approveSign" data-action="submit">
                <i class="ti ti-send me-1"></i> Submit SR
            </button>
            ...
            <button type="button" class="btn btn-outline-danger btn-sm rounded-pill shadow-sm px-3 fw-bold openCancelAction"
            ...
            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($isAdmin || $user->hasPermission('sr.revision')): ?>
            <button type="button" class="btn btn-warning btn-sm rounded-pill shadow-sm px-3 fw-bold approveSign"
                data-bs-toggle="modal" data-bs-target="#modalApprove" data-action="revision">
                <i class="ti ti-refresh me-1"></i> Revision
            </button>
            <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($isAdmin || $user->hasPermission('sr.reject')): ?>
    ...
    <div class="row">
        ...
        <div class="col-12 mb-4 d-none">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-0">
                    ...
                    <li class="breadcrumb-item"><a href="<?php echo e(route('settlement-reports.index')); ?>" class="text-decoration-none text-uppercase">SETTLEMENT REPORT</a></li>
        ...
        <div class="col-12">
            <div class="card p-4 border-0 shadow-sm">
                ...
                <div class="d-flex justify-content-between align-items-start mb-4 border-bottom pb-3" data-html2canvas-ignore="true">
                    <div>
                    ...
                    <div class="d-flex align-items-center gap-3">
                        ...
                        <div class="d-flex gap-2">
                            <a href="<?php echo e(route('settlement-reports.index')); ?>" class="btn btn-secondary btn-sm" data-html2canvas-ignore="true">
                            ...
                            <a href="<?php echo e(route('settlement-reports.payment', hashid_encode($sr->id_pr, 'pr'))); ?>" class="btn btn-primary btn-sm">Payment</a>
                            <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                            ...
                            <button type="button" wire:click="$dispatch('open-sr-form', { id: <?php echo e($sr->id_pr); ?> })" class="btn btn-warning btn-sm" data-html2canvas-ignore="true">
                            ...
                            <button class="btn btn-danger btn-sm" wire:click="deleteSr" onclick="return confirm('Hapus Data?')" data-html2canvas-ignore="true">Delete</button>
                            <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                        </div>
                        
                        <button class="btn btn-success btn-sm d-flex align-items-center justify-content-center" style="height: 38px; width: 38px;" data-html2canvas-ignore="true">
                ...
                <div class="d-flex align-items-center mb-4">
                    ...
                    <div style="width: 30%;">
                        <?php
                        $logoPath = $sr->company->logo ?? 'logo.png';
                        ...
                        <img src="<?php echo e(asset('assets/companies/logos/' . $logoPath)); ?>" class="img-fluid" alt="Logo" style="max-height: 120px; object-fit: contain;">
                        <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                    </div>
                    
                    <div class="text-center" style="flex: 1;">
                        <h4 style="font-weight: 700; color: #3b5998; text-transform: uppercase; margin-bottom: 5px;">
                            <?php echo e($sr->company->company_name ?? $sr->company->company ?? config('app.name')); ?>
                ...
                <div class="row pt-2">
                    <div class="alert alert-success d-flex align-items-center mb-4" role="alert" style="padding: 8px 15px; background-color: #dcf2d1; border-color: #c8e6b9; border-radius: 4px;">
                        <div style="text-decoration: underline; font-weight:600; color: #3b5a2b; font-size: 13px;">SETTLEMENT REPORT DETAIL</div>
                        <div class="mx-3">
                            <span class="badge bg-<?php echo e($sbadge['color'] == 'warning' ? 'warning' : $sbadge['color']); ?>" style="font-size:11px; padding: 4px 8px; <?php echo e($sbadge['color'] == 'warning' ? 'color: #000;' : ''); ?>"><?php echo e($sbadge['label']); ?></span>
                        </div>
                    </div>

                    <!-- Data Info Sebelah Kiri -->
                    <div class="form-group col-md-6">
                        <div class="mb-3 d-flex" style="font-size: 13px;">
                            <div class="col-3 fw-bold">SUBJECT</div>
                            <div class="col-1 text-end">:&nbsp;</div>
                            <div class="col-8" style="text-align: justify;"><?php echo e($sr->subject); ?></div>
                        </div>
                        <div class="mb-3 d-flex" style="font-size: 13px;">
                            <div class="col-3 fw-bold">SR CATEGORY</div>
                            <div class="col-1 text-end">:&nbsp;</div>
                            <div class="col-8">Settlement</div>
                        </div>
                        <div class="mb-3 d-flex" style="font-size: 13px;">
                            <div class="col-3 fw-bold">CATEGORY</div>
                            <div class="col-1 text-end">:&nbsp;</div>
                            <div class="col-8"><?php echo e($sr->costCategory->cost_category ?? $sr->pr->costCategory->cost_category ?? '-'); ?></div>
                        </div>
                        <div class="mb-3 d-flex" style="font-size: 13px;">
                            <div class="col-3 fw-bold">EXPENSE</div>
                            <div class="col-1 text-end">:&nbsp;</div>
                            <div class="col-8"><?php echo e($sr->costType->cost_type ?? $sr->pr->costType->cost_type ?? '-'); ?></div>
                        </div>
                        <div class="mb-3 d-flex align-items-center" style="font-size: 13px;">
                            ...
                            <div class="col-8">
                                <?php
                                $canManageLoan = ($isAdmin || $user->hasPermission('loan.view')) && !in_array($sr->status, [11]);
                                ?>
                                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($sr->loan && $sr->loan->loan): ?>
                                ...
                                <a href="#" data-bs-toggle="modal" data-bs-target="#modalLoan" class="text-decoration-none text-dark btn btn-success fw-bold btn-sm">
                                    <?php echo e($sr->loan->loan); ?>

                                </a>
                                <?php else: ?>
                                <span class="text-dark btn btn-success fw-bold btn-sm disabled" style="opacity: 1;">
                                    <?php echo e($sr->loan->loan); ?>
                                ...
                                <?php else: ?>
                                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($canManageLoan): ?>
                                <a href="#" data-bs-toggle="modal" data-bs-target="#modalLoan" class="text-decoration-none btn btn-primary fw-bold btn-sm">
                                    ADD
                                </a>
                                <?php else: ?>
                                -
                    ...
                    <!-- Data Info Sebelah Kanan -->
                    <div class="form-group col-md-6">
                        <div class="mb-3 d-flex" style="font-size: 13px;">
                            <div class="col-3 fw-bold">SETTLEMENT DATE</div>
                            <div class="col-1 text-end">:&nbsp;</div>
                            <div class="col-8"><?php echo e($sr->created_at?->format('d F Y')); ?></div>
                        </div>
                        <div class="mb-3 d-flex" style="font-size: 13px;">
                            <div class="col-3 fw-bold">PAYMENT DUE DATE</div>
                            <div class="col-1 text-end">:&nbsp;</div>
                            <div class="col-8"><?php echo e($sr->pr->payment_due_date ? \Carbon\Carbon::parse($sr->pr->payment_due_date)->format('d F Y') : '-'); ?></div>
                        </div>
                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(in_array($sr->pr->id_doc_type ?? 0, [2,3]) || $sr->id_doc_type == 3): ?>
                        <div class="mb-3 d-flex" style="font-size: 13px;">
                            <div class="col-3 fw-bold">ES SETTLEMENT DATE</div>
                            <div class="col-1 text-end">:&nbsp;</div>
                            <div class="col-8"><?php echo e($sr->pr->est_settlement_date ? \Carbon\Carbon::parse($sr->pr->est_settlement_date)->format('d F Y') : '-'); ?></div>
                        </div>
                        <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                        <div class="mb-3 d-flex" style="font-size: 13px;">
                            <div class="col-3 fw-bold">PR NUMBER</div>
                            <div class="col-1 text-end">:&nbsp;</div>
                            <div class="col-8"><?php echo e($sr->pr->pr_number ?? '-'); ?></div>
                        </div>
                        <div class="mb-3 d-flex" style="font-size: 13px;">
                            <div class="col-3 fw-bold">BRANCH</div>
                            <div class="col-1 text-end">:&nbsp;</div>
                            <div class="col-8"><?php echo e($sr->branch->branch ?? $sr->pr->branch->branch ?? '-'); ?></div>
                        </div>
                        <div class="mb-3 d-flex" style="font-size: 13px;">
                            <div class="col-3 fw-bold">DEPARTEMENT</div>
                            <div class="col-1 text-end">:&nbsp;</div>
                            <div class="col-8"><?php echo e($sr->departement->departement ?? $sr->pr->departement->departement ?? '-'); ?></div>
                        </div>
                        <div class="mb-3 d-flex" style="font-size: 13px;">
                            <div class="col-3 fw-bold">CATEGORY</div>
                            <div class="col-1 text-end">:&nbsp;</div>
                            <div class="col-8">General</div> 
                        </div>
                        <div class="mb-3 d-flex" style="font-size: 13px;">
                            <div class="col-3 fw-bold">EXPENSE</div>
                            <div class="col-1 text-end">:&nbsp;</div>
                            <div class="col-8">Non Routine General</div> 
                        </div>
                    </div>
                    ...
                    <div class="form-group col-md-6">
                        <div class="mb-3 d-flex" style="font-size: 13px;">
                            ...
                            <div class="col-1 text-end">:&nbsp;</div>
                            <div class="col-8"><?php echo e($sr->vendor->vendor ?? $sr->pr->vendor->vendor ?? '-'); ?></div>
                        </div>
                        <div class="mb-3 d-flex" style="font-size: 13px;">
                            <div class="col-3 fw-bold">NO. INVOICE</div>
                            <div class="col-1 text-end">:&nbsp;</div>
                            <div class="col-8"><?php echo e($sr->no_invoice ?: ($sr->pr->no_invoice ?: '-')); ?></div>
                        </div>
                        <div class="mb-3 d-flex" style="font-size: 13px;">
                            <div class="col-3 fw-bold">PO NUMBER</div>
                            <div class="col-1 text-end">:&nbsp;</div>
                            <div class="col-8"><?php echo e($sr->pr->po_number ?? '-'); ?></div>
                        </div>
                        <div class="mb-3 d-flex" style="font-size: 13px;">
                            <div class="col-3 fw-bold">EMAIL</div>
                            <div class="col-1 text-end">:&nbsp;</div>
                            <div class="col-8">
                                <span class="d-block"><?php echo e($sr->emailVendor->email ?? $sr->pr->emailVendor->email ?? '-'); ?></span>
                                <span class="text-muted" style="font-size: 11px;"><?php echo e($sr->emailUser->email ?? $sr->pr->emailUser->email ?? ''); ?></span>
                            </div>
                        </div>
                        <div class="mb-3 d-flex" style="font-size: 13px;">
                            <?php
                            $vendorData = $sr->vendor ?? $sr->pr->vendor ?? null;
                            $npwp = !empty($vendorData->npwp) ? decrypt_legacy($vendorData->npwp) : '';
                            $nik = !empty($vendorData->nik) ? decrypt_legacy($vendorData->nik) : '';
                            if (!empty($npwp) && !empty($nik)) $npwp_nik = $npwp . ' - ' . $nik;
                            elseif (!empty($npwp)) $npwp_nik = $npwp;
                            elseif (!empty($nik)) $npwp_nik = $nik;
                            else $npwp_nik = '-';
                            ?>
                    ...
                    <div class="form-group col-md-6">
                        <div class="mb-3 d-flex" style="font-size: 13px;">
                            ...
                            <div class="col-1 text-end">:&nbsp;</div>
                            <div class="col-8"><?php echo e(($sr->payment_method ?? $sr->pr->payment_method) == 1 ? 'Transfer' : 'Cash'); ?></div>
                        </div>
                        <div class="mb-3 d-flex" style="font-size: 13px;">
                            <div class="col-3 fw-bold">PAYMENT TYPE</div>
                            <div class="col-1 text-end">:&nbsp;</div>
                            <div class="col-8"><?php echo e($sr->pr->payment_type_pr == 1 ? 'Parsial' : 'Full Payment'); ?></div>
                        </div>

                        <div class="mb-3 d-flex" style="font-size: 13px;">
                            <div class="col-3 fw-bold">BANK NAME</div>
                            <div class="col-1 text-end">:&nbsp;</div>
                            <div class="col-8"><?php echo e($sr->nama_bank ?: ($sr->norekVendor->nama_bank ?? ($sr->pr->nama_bank ?: ($sr->pr->norekVendor->nama_bank ?? '-')))); ?></div>
                        </div>
                        <div class="mb-3 d-flex" style="font-size: 13px;">
                            <div class="col-3 fw-bold">ACCOUNT NAME</div>
                            <div class="col-1 text-end">:&nbsp;</div>
                            <div class="col-8"><?php echo e($sr->nama_penerima ?: ($sr->norekVendor->nama_penerima ?? ($sr->pr->nama_penerima ?: ($sr->pr->norekVendor->nama_penerima ?? '-')))); ?></div>
                        </div>
                        <div class="mb-3 d-flex" style="font-size: 13px;">
                            <div class="col-3 fw-bold">ACCOUNT NUMBER</div>
                            <div class="col-1 text-end">:&nbsp;</div>
                            <div class="col-8"><?php echo e($sr->norek ?: ($sr->norekVendor->norek ?? ($sr->pr->norek ?: ($sr->pr->norekVendor->norek ?? '-')))); ?></div>
                        </div>
                    </div>
                </div>
                ...
                <div class="card-header align-items-center bg bg-transparent d-flex">
                    ...
                    <?php
                    $canAddDetail = $isAdmin || ($isOwner && $user->hasPermission('sr_detail.create'));
                    $canEditDetail = $isAdmin || ($isOwner && $user->hasPermission('sr_detail.edit'));
                    $canViewDetail = $isAdmin || $isOwner || $user->hasPermission('sr_detail.view');
                    ?>

                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($canAddDetail && in_array($sr->status, [null, 0, 12])): ?>
                    <div class="flex-shrink-0">
                        <button class="btn btn-primary" wire:click="$dispatchTo('settlement-reports.form-detail-modal', 'openModal')">ADD</button>
                    </div>
                    <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                </div>
                    ...
                    <table class="table table-striped table-bordered nowrap">
                        <thead class="text-center" style="background-color: green; color: white;">
                            <tr>
                                ...
                                <th>PPH</th>
                                <th>AMOUNT</th>
                                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($canEditDetail && in_array($sr->status, [null, 0, 12])): ?><th data-html2canvas-ignore="true">ACTION</th><?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                            </tr>
                        </thead>
                        <tbody>
                            <?php $no = 1; ?>
                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__empty_1 = true; $__currentLoopData = $sr->details; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoop($loop->index); ?><?php endif; ?>
                            <tr>
                                <td class="text-center"><?php echo e($no++); ?></td>
                                <td class="text-wrap" style="max-width: 360px; cursor: <?php echo e($canViewDetail ? 'pointer' : 'default'); ?>;"
                                    <?php if($canViewDetail): ?>
                                    wire:click="$dispatchTo('settlement-reports.form-detail-modal', 'openModal', { id: <?php echo e($item->id_detail_sr); ?>, viewOnly: true })"
                                    <?php endif; ?>>
                                    <?php echo e($item->detail); ?>

                                </td>
                                <td class="text-wrap text-center" style="max-width: 360px;"><?php echo e($item->bl_number ?? '-'); ?></td>
                                <td class="text-center"><?php echo e(number_format($item->qty, 2, ',', '.')); ?></td>
                                <td class="text-center"><?php echo e($item->uom->uom ?? '-'); ?></td>
                                <td class="text-center"><?php echo e($symbol); ?> <?php echo e(number_format($item->price, 2, ',', '.')); ?></td>
                                <td class="text-center"><?php echo e($symbol); ?> <?php echo e(number_format($item->discount, 2, ',', '.')); ?></td>
                                <td class="text-center">
                                    ...
                                    <?php echo e($item->taxSatu->tax); ?> (<?php echo e(number_format($item->taxSatu->tax_persen * 100, 2, ',', '.')); ?>%)<br>
                                ...
                                <td class="text-center">
                                    <?php
                                    $labels = [];
                                    if(!empty($item->progresif) && $item->progresif == 1) $labels[] = 'Progresif';
                                    if(!empty($item->gross) && $item->gross == 1) $labels[] = 'PPh Gross Up';
                                    ...
                                    <?php echo e($item->taxDua->tax ?? ''); ?> (<?php echo e(number_format(($item->taxDua->tax_persen ?? 0) * 100, 2, ',', '.')); ?>%)<br>
                                    <?php echo e($symbol); ?> <?php echo e(number_format($item->tax2, 2, ',', '.')); ?>

                                    <?php else: ?> -
                                    <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                                </td>
                                <td class="fw-bold"><?php echo e($symbol); ?> <?php echo e(number_format($item->ammount, 2, ',', '.')); ?></td>
                                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($canEditDetail && in_array($sr->status, [null, 0, 12])): ?>
                                <td class="text-center" data-html2canvas-ignore="true">
                                    <span>
                                        <button class="ti ti-edit fs-3 btn btn-link p-0"
                                            wire:click="$dispatchTo('settlement-reports.form-detail-modal', 'openModal', { id: <?php echo e($item->id_detail_sr); ?> })"></button>
                                    </span>
                                    <span style="color:red;">
                                        <button class="ti ti-trash fs-3 btn btn-link p-0"
                                            onclick="showConfirm({
                                                ...
                                                type: 'danger',
                                                onConfirm: () => Livewire.dispatch('delete-detail', { id: <?php echo e($item->id_detail_sr); ?> })
                                            })"></button>
                                    </span>
                                </td>
                                <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                            ...
                            <?php $showActionColumn = $canEditDetail && in_array($sr->status, [null, 0, 12]); ?>
                            <tr>
                                <td colspan="<?php echo e($showActionColumn ? 11 : 10); ?>" class="text-center py-3">Tidak ada item.</td>
                            </tr>
                            <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                        </tbody>
                        <tfoot>
                            <tr>
                                <th colspan="9" style="text-align: right; background-color: #f8f9fa;">ADDITIONAL DISCOUNT</th>
                                <th colspan="<?php echo e(($canEditDetail && in_array($sr->status, [null, 0, 12])) ? 2 : 1); ?>"><?php echo e($symbol); ?> <?php echo e(number_format($sr->additional_discount, 2, ',', '.')); ?></th>
                            </tr>
                            <tr>
                                <th colspan="9" style="text-align: right; background-color: #d1e7dd;">GRAND TOTAL</th>
                                <th colspan="<?php echo e(($canEditDetail && in_array($sr->status, [null, 0, 12])) ? 2 : 1); ?>" class="fw-bold"><?php echo e($symbol); ?> <?php echo e(number_format($this->grandTotal, 2, ',', '.')); ?></th>
                            </tr>
                            <tr>
                                <th colspan="9" style="text-align: right; background-color: #d1e7dd;">TOTAL RECEIPTS</th>
                                <th colspan="<?php echo e(($canEditDetail && in_array($sr->status, [null, 0, 12])) ? 2 : 1); ?>"><?php echo e($symbol); ?> <?php echo e(number_format($this->receipt, 2, ',', '.')); ?></th>
                            </tr>
                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($this->receipt != $this->grandTotal): ?>
                            <tr>
                                <th colspan="9" style="text-align: right; background-color: #fff3cd;">REFUND</th>
                                <th colspan="<?php echo e(($canEditDetail && in_array($sr->status, [null, 0, 12])) ? 2 : 1); ?>"><?php echo e($symbol); ?> <?php echo e(number_format($this->refund, 2, ',', '.')); ?></th>
                            </tr>
                            <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                            <tr>
                                <th colspan="9" style="text-align: right; background-color: #fff3cd;" class="fw-bold">BALANCE</th>
                                <th colspan="<?php echo e(($canEditDetail && in_array($sr->status, [null, 0, 12])) ? 2 : 1); ?>" class="fw-bold"><?php echo e($symbol); ?> <?php echo e(number_format($this->balance, 2, ',', '.')); ?></th>
                            </tr>
                        </tfoot>
                    </table>
                </div>
                ...
                <div class="card border-0 shadow-sm theme-responsive-card">
                    <div class="card-body p-4">
                        ...
                        $canManageAtt = auth()->user()->level === 1 ||
                        (auth()->user()->id_user == $sr->id_user && auth()->user()->hasPermission('sr_attachment.create'));

                        // 2. Allowed Status?
                        $isAllowedStatus = in_array($sr->status, [0, 8, 12, 14]); // Added 14 as per earlier discussion

                        // 3. Can View?
                        $canViewAtt = auth()->user()->level === 1 ||
                        auth()->user()->id_user == $sr->id_user ||
                        auth()->user()->hasPermission('sr_attachment.view');
                        ?>
                        <div class="card-header align-items-center bg-transparent d-flex px-0 py-3 mt-4 border-bottom">
                            ...
                            <div class="flex-shrink-0">
                                ...
                                <button type="button" class="btn btn-primary no-print-btn" data-bs-toggle="modal" data-bs-target="#modalAddAttachment" data-html2canvas-ignore>
                                    ADD
                                </button>
                                <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                            </div>
                        ...
                        <div class="row g-3 py-3" style="font-size: 15px;">
                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__currentLoopData = $sr->attachments; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $file): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoop($loop->index); ?><?php endif; ?>
                            <?php
                            $canEditDeleteAtt = false;
                            $attHash = hashid_encode($file->id_attachment_sr, 'attachment-sr');
                            if ($isAllowedStatus && $canManageAtt) {
                            $canEditDeleteAtt = true;
                            if ($sr->status == 8) {
                            $enter8Date = App\Models\SignTransaction::where('id_pr', $sr->id_pr)
                            ->where('status', 8)
                            ->latest()
                            ->value('updated_at');
                            if ($enter8Date && $file->created_at < $enter8Date) {
                                ...
                                <div class="form-check form-check-inline d-flex align-items-center mb-0">
                                    <input type="checkbox" class="form-check-input input-primary me-2" checked onclick="return false;" style="width: 1.2rem; height: 1.2rem;">
                                    <label class="form-check-label text-justify flex-grow-1 fw-bold text-uppercase" style="cursor:pointer; font-size: 0.9rem;">
                                        <a href="javascript:void(0)"
                                            data-url="<?php echo e(asset('assets/attachmentsr/' . $file->filename)); ?>"
                                            data-type="<?php echo e($file->attachment->attachment ?? 'GENERAL'); ?>"
                                            data-note="<?php echo e($file->note); ?>"
                                            data-delete="<?php echo e(route('settlement-reports.attachment.delete', $attHash)); ?>"
                                            data-update="<?php echo e(route('settlement-reports.attachment.update', $attHash)); ?>"
            ...
            <div class="modal fade" id="modalLoan" tabindex="-1" aria-labelledby="modalLoanLabel" aria-hidden="true" wire:ignore.self>
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            <div class="bg-light p-3 border rounded mb-3">
                                ...
                                <div class="small">
                                    <strong>Attachment:</strong> <span id="previewAttachment" class="text-dark"></span>
                                </div>
                            </div>
                            <div id="previewBody" class="text-center p-2 rounded" style="min-height: 400px; background: #fff; border: 1px solid #dee2e6;">
                        ...
                        <div class="modal-footer border-top-0 d-flex justify-content-end gap-2">
                            ...
                            <button type="button" id="editBtn" class="btn btn-warning px-4" style="display:none;">
                                <i class="ti ti-edit me-1"></i>Edit
                            </button>
                            <button type="button" id="deleteBtn" class="btn btn-danger px-4" style="display:none;">
                        ...
                        <form id="formEditAttachment" method="POST" action="" enctype="multipart/form-data">
                            ...
                            <div class="modal-body">
                                ...
                                <div class="mb-3">
                                    ...
                                    <select class="form-select" name="id_attachment" id="edit_id_attachment" required>
                                        ...
                                        <option value="<?php echo e($type->id_attachment); ?>"><?php echo e($type->attachment); ?></option>
                                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::endLoop(); ?><?php endif; ?><?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::closeLoop(); ?><?php endif; ?>
                                    </select>
                                </div>
                                <div class="mb-3">
                                    ...
                                    <div class="btn-group w-100 mb-2" role="group">
                                        <input type="radio" class="btn-check" name="edit_upload_mode" id="editModeUpload" value="upload" checked>
                                        <label class="btn btn-outline-primary btn-sm" for="editModeUpload"><i class="ti ti-upload me-1"></i> Upload</label>
                                        <input type="radio" class="btn-check" name="edit_upload_mode" id="editModeCamera" value="camera">
                                        <label class="btn btn-outline-primary btn-sm" for="editModeCamera"><i class="ti ti-camera me-1"></i> Camera</label>
                                    </div>

                                    <div id="edit_upload_container">
                                        <input type="file" class="form-control" name="filename" id="input_edit_file" accept="image/*,application/pdf">
                                    </div>
                                    <div id="edit_camera_container" class="d-none">
                                        <div class="camera-wrapper bg-light rounded p-2 text-center border">
                                            <video id="video_edit" width="100%" height="auto" autoplay class="rounded mb-2 d-none"></video>
                                            <canvas id="canvas_edit" class="d-none"></canvas>
                                            <div class="d-flex justify-content-center gap-2">
                                                <button type="button" id="start-camera-edit" class="btn btn-sm btn-info rounded-pill px-3">
                                                    <i class="ti ti-video me-1"></i> Start Camera
                                                </button>
                                                <button type="button" id="take-photo-edit" class="btn btn-sm btn-success rounded-pill px-3 d-none">
                                    ...
                                    <input type="hidden" name="captured_photo_mime" id="captured_photo_mime_edit">
                                    <div class="small text-muted mt-1">Maksimal 5MB. Format: JPG, JPEG, PNG, PDF.</div>
                                </div>
                                <div id="edit_gallery_container" class="mt-2 d-none">
                                    <label class="form-label small fw-bold">Captured Photos:</label>
                                    <div class="d-flex flex-wrap gap-2 p-2 border rounded bg-light" id="edit_gallery_list">
                                        <!-- Photos will be appended here -->
                                    </div>
                                    <div class="small text-muted mt-1">Single photo will be saved as image, multiple as PDF.</div>
                                </div>
                                <div id="edit_preview_container" class="mt-2 d-none">
                                    <label class="form-label small fw-bold" id="edit_preview_label">Preview:</label>
                                    <div class="border rounded p-2 text-center bg-light position-relative">
                                        <img id="edit_preview_img" src="" class="img-fluid rounded d-none" style="max-height: 200px;">
                                        <div id="edit_preview_pdf" class="d-none">
                                            <i class="ti ti-file-text fs-1 text-danger"></i>
                                            <div class="small fw-bold text-muted mt-1" id="edit_pdf_name">Merged_Attachment.pdf</div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="modal-footer">
                                ...
                                <button type="submit" id="btnUploadEdit" class="btn btn-warning px-4">Update</button>
                            </div>
                        </form>
                    </div>
                </div>
            ...
            <div class="modal fade" id="modalAddAttachment" tabindex="-1" aria-hidden="true">
                <div class="modal-dialog modal-lg modal-dialog-centered">
                    <div class="modal-content">
                        <form id="formAddAttachment" method="POST" action="<?php echo e(route('settlement-reports.attachment.store', $srHash)); ?>" enctype="multipart/form-data">
                            <?php echo csrf_field(); ?>
                            <div class="modal-header">
                            ...
                            <div class="modal-body">
                                ...
                                <div class="mb-3">
                                    ...
                                    <select class="form-select select2" name="id_attachment">
                                        ...
                                        <option value="<?php echo e($att->id_attachment); ?>"><?php echo e($att->attachment); ?></option>
                                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::endLoop(); ?><?php endif; ?><?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::closeLoop(); ?><?php endif; ?>
                                    </select>
                                </div>
                                <div class="mb-3">
                                    ...
                                    <div class="btn-group w-100 mb-2" role="group">
                                        <input type="radio" class="btn-check" name="add_upload_mode" id="addModeUpload" value="upload" checked>
                                        <label class="btn btn-outline-primary btn-sm" for="addModeUpload"><i class="ti ti-upload me-1"></i> Upload</label>
                                        <input type="radio" class="btn-check" name="add_upload_mode" id="addModeCamera" value="camera">
                                        <label class="btn btn-outline-primary btn-sm" for="addModeCamera"><i class="ti ti-camera me-1"></i> Camera</label>
                                    </div>

                                    <div id="add_upload_container">
                                        <input type="file" class="form-control" name="filename" id="input_add_file" accept="image/*,application/pdf">
                                    </div>
                                    <div id="add_camera_container" class="d-none">
                                        <div class="camera-wrapper bg-light rounded p-2 text-center border">
                                            <video id="video_add" width="100%" height="auto" autoplay class="rounded mb-2 d-none"></video>
                                            <canvas id="canvas_add" class="d-none"></canvas>
                                            <div class="d-flex justify-content-center gap-2">
                                                <button type="button" id="start-camera-add" class="btn btn-sm btn-info rounded-pill px-3">
                                                    <i class="ti ti-video me-1"></i> Start Camera
                                                </button>
                                                <button type="button" id="take-photo-add" class="btn btn-sm btn-success rounded-pill px-3 d-none">
                                                    <i class="ti ti-camera me-1"></i> Capture
                                                </button>
                                            </div>
                                        </div>
                                    ...
                                    <input type="hidden" name="captured_photo_mime" id="captured_photo_mime_add">
                                </div>
                                <div id="add_gallery_container" class="mt-2 d-none">
                                    <label class="form-label small fw-bold">Captured Photos:</label>
                                    <div class="d-flex flex-wrap gap-2 p-2 border rounded bg-light" id="add_gallery_list">
                                        <!-- Photos will be appended here -->
                                    </div>
                                    <div class="small text-muted mt-1">Single photo will be saved as image, multiple as PDF.</div>
                                </div>
                                <div id="add_preview_container" class="mt-2 d-none">
                                    <label class="form-label small fw-bold" id="add_preview_label">Preview:</label>
                                    <div class="border rounded p-2 text-center bg-light position-relative">
                                        <img id="add_preview_img" src="" class="img-fluid rounded d-none" style="max-height: 200px;">
                                        <div id="add_preview_pdf" class="d-none">
                                            <i class="ti ti-file-text fs-1 text-danger"></i>
                                            <div class="small fw-bold text-muted mt-1" id="add_pdf_name">Merged_Attachment.pdf</div>
                                        </div>
                                    </div>
                                </div>
                            </div>
            ...
            <div class="row text-uppercase fw-bold" style="color: #002b00;">
                <div class="row d-flex justify-content-center flex-wrap text-center">
                    ...
                    <div class="col-12 col-sm-6 col-md-3 mb-4">
                        ...
                        <div class="border-bottom mx-auto position-relative image-container" style="height: 100px; width: 120px; border-color: #333 !important;">
                            <?php $s1 = $sign(1); ?>
                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($s1): ?>
                            <?php
                            $qrText1 = "SR: " . ($sr->pr_number ?? 'DRAFT') . "\nSubmitted By: " . ($s1->user->name ?? '-') . "\nRole: APPLICANT\nDate: " . $s1->created_at->format('d/m/Y H:i');
                            ?>
                            <div class="pt-2">
                                <img src="<?php echo e($this->getQr($qrText1)); ?>" class="signature-img" style="height: 80px; width: 80px; object-fit: contain; display: block; margin: 0 auto;">
                            </div>
                            <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($canCancelSubmit): ?>
                            <button type="button" class="btn btn-outline-danger btn-xs position-absolute top-50 start-50 translate-middle no-print-btn openCancelAction"
                                data-action="cancelSubmit" data-bs-toggle="modal" data-bs-target="#modalCancelAction" data-html2canvas-ignore="true">Cancel</button>
                            <?php elseif($canSubmit): ?>
                            <div class="d-flex justify-content-center no-print-btn mt-3" data-html2canvas-ignore="true">
                                <button type="button" data-action="submit" class="btn btn-outline-primary btn-xs px-3 fw-bold approveSign">SUBMIT</button>
                            </div>
                            <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                        </div>
                        <div class="mt-2 fw-bold small <?php echo e($s1 ? 'text-uppercase' : ''); ?>" style="font-size: 14px;"><?php echo e($s1?->user?->name ?? 'User'); ?></div>
                    </div>
                    ...
                    <div class="col-12 col-sm-6 col-md-3 mb-4">
                        ...
                        <div class="border-bottom mx-auto position-relative image-container" style="height: 100px; width: 120px; border-color: #333 !important;">
                            ...
                            <?php
                            $qrText2 = "SR: " . ($sr->pr_number ?? 'DRAFT') . "\nVerified By: " . ($s2->user->name ?? '-') . "\nRole: DEPARTMENT HEAD\nDate: " . $s2->created_at->format('d/m/Y H:i');
                            ?>
                            <div class="pt-2">
                                <img src="<?php echo e($this->getQr($qrText2)); ?>" class="signature-img" style="height: 80px; width: 80px; object-fit: contain; display: block; margin: 0 auto;">
                            </div>
                            <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                            ...
                            <button type="button" class="btn btn-outline-danger btn-xs position-absolute top-50 start-50 translate-middle no-print-btn openCancelAction"
                                data-action="cancelApproval" data-bs-toggle="modal" data-bs-target="#modalCancelAction" data-html2canvas-ignore="true">Cancel</button>
                            <?php elseif($status == 1 && $canApprove): ?>
                            <div class="d-flex flex-row justify-content-center gap-1 pt-4 no-print-btn" data-html2canvas-ignore="true">
                                <button type="button" class="btn btn-success btn-icon btn-sm rounded-circle approveSign" data-role="dept" data-action="approve" title="Approve"><i class="ti ti-check fs-6"></i></button>
                                <button type="button" class="btn btn-danger btn-icon btn-sm rounded-circle approveSign" data-role="dept" data-action="reject" title="Reject"><i class="ti ti-x fs-6"></i></button>
                                <button type="button" class="btn btn-warning btn-icon btn-sm rounded-circle approveSign" data-role="dept" data-action="revision" title="Revision"><i class="ti ti-refresh fs-6"></i></button>
                            </div>
                            <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                        </div>
                        <div class="mt-2 fw-bold small <?php echo e($s2 ? 'text-uppercase' : ''); ?>" style="font-size: 14px;"><?php echo e($s2?->user?->name ?? 'Department Head'); ?></div>
                    </div>
                    ...
                    <div class="col-12 col-sm-6 col-md-3 mb-4">
                        ...
                        <div class="border-bottom mx-auto position-relative image-container" style="height: 100px; width: 120px; border-color: #333 !important;">
                            <?php
                            $dirStatus = 3; // Director approval selalu menyimpan SignTransaction dengan status=3
                            $s3 = $sign($dirStatus);
                            ?>
                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($s3): ?>
                            <?php
                            $qrText3 = "SR: " . ($sr->pr_number ?? 'DRAFT') . "\nApproved By: " . ($s3->user->name ?? '-') . "\nRole: DIRECTOR\nDate: " . $s3->created_at->format('d/m/Y H:i');
                            ?>
                            <div class="pt-2">
                                <img src="<?php echo e($this->getQr($qrText3)); ?>" class="signature-img" style="height: 80px; width: 80px; object-fit: contain; display: block; margin: 0 auto;">
                            </div>
                            <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                            ...
                            <div class="d-flex flex-row justify-content-center gap-1 pt-4 no-print-btn" data-html2canvas-ignore="true">
                                <button type="button" class="btn btn-success btn-icon btn-sm rounded-circle approveSign" data-role="director" data-action="approve" title="Approve"><i class="ti ti-check fs-6"></i></button>
                                <button type="button" class="btn btn-danger btn-icon btn-sm rounded-circle approveSign" data-role="director" data-action="reject" title="Reject"><i class="ti ti-x fs-6"></i></button>
                                <button type="button" class="btn btn-warning btn-icon btn-sm rounded-circle approveSign" data-role="director" data-action="revision" title="Revision"><i class="ti ti-refresh fs-6"></i></button>
                            </div>
                            <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                        </div>
                        <div class="mt-2 fw-bold small <?php echo e($s3 ? 'text-uppercase' : ''); ?>" style="font-size: 14px;"><?php echo e($s3?->user?->name ?? 'Director'); ?></div>
                    </div>

                    
                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(in_array($sr->id_doc_type, [1,3])): ?>
                    <div class="col-12 col-sm-6 col-md-3 mb-4">
                        <div class="fw-bold mb-1 small text-uppercase" style="font-size: 14px;">Checked By</div>
                        <div class="border-bottom mx-auto position-relative image-container" style="height: 100px; width: 120px; border-color: #333 !important;">
                            <?php
                            $s4 = $sign(4); // Accounting approval selalu menyimpan SignTransaction dengan status=4
                            $s5 = $sign(5); // Pre-compute Finance sign untuk cek cancel button
                            ?>
                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($s4): ?>
                            <?php
                            $qrText4 = "SR: " . ($sr->pr_number ?? 'DRAFT') . "\nChecked By: " . ($s4->user->name ?? '-') . "\nRole: ACCOUNTING\nDate: " . $s4->created_at->format('d/m/Y H:i');
                            ?>
                            <div class="pt-2">
                                <img src="<?php echo e($this->getQr($qrText4)); ?>" class="signature-img" style="height: 80px; width: 80px; object-fit: contain; display: block; margin: 0 auto;">
                            </div>
                            <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($status == 4 && $canCancelApprove && !$s5): ?>
                            <button type="button" class="btn btn-outline-danger btn-xs position-absolute top-50 start-50 translate-middle no-print-btn openCancelAction"
                                data-action="cancelApproval" data-bs-toggle="modal" data-bs-target="#modalCancelAction" data-html2canvas-ignore="true">Cancel</button>
                            <?php elseif($status == 3 && $canApprove): ?>
                            <div class="d-flex flex-row justify-content-center gap-1 pt-4 no-print-btn" data-html2canvas-ignore="true">
                                <button type="button" class="btn btn-success btn-icon btn-sm rounded-circle approveSign" data-role="accounting" data-action="approve" title="Approve"><i class="ti ti-check fs-6"></i></button>
                                <button type="button" class="btn btn-danger btn-icon btn-sm rounded-circle approveSign" data-role="accounting" data-action="reject" title="Reject"><i class="ti ti-x fs-6"></i></button>
                                <button type="button" class="btn btn-warning btn-icon btn-sm rounded-circle approveSign" data-role="accounting" data-action="revision" title="Revision"><i class="ti ti-refresh fs-6"></i></button>
                            </div>
                            <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                        </div>
                        <div class="mt-2 fw-bold small <?php echo e($s4 ? 'text-uppercase' : ''); ?>" style="font-size: 14px;"><?php echo e($s4?->user?->name ?? 'Accounting'); ?></div>
                    </div>
                    <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
        ...
        <div class="row">
            <div class="alert alert-light border text-center fw-bold py-1 my-2 text-uppercase" style="font-size: 11px; letter-spacing: 1px; color: #555;">PAYMENT PROCESSING</div>
                ...
                <div class="row text-uppercase fw-bold" style="color: #002b00;">
                    <div class="row d-flex justify-content-center flex-wrap text-center">
                        ...
                        <div class="col-12 col-sm-6 col-md-4 mb-4">
                            <div class="fw-bold mb-1 small text-uppercase" style="font-size: 14px;">Prepared By</div>
                            <div class="border-bottom mx-auto position-relative image-container" style="height: 100px; width: 120px; border-color: #333 !important;">
                                <?php $s5 = $sign(5); ?>
                                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($s5): ?>
                                <?php
                                $qrText5 = "SR: " . ($sr->pr_number ?? 'DRAFT') . "\nPrepared By: " . ($s5->user->name ?? '-') . "\nRole: FINANCE STAFF\nDate: " . $s5->created_at->format('d/m/Y H:i');
                                ?>
                                <div class="pt-2">
                                    <img src="<?php echo e($this->getQr($qrText5)); ?>" class="signature-img" style="height: 80px; width: 80px; object-fit: contain; display: block; margin: 0 auto;">
                                </div>
                                <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                                ...
                                <div class="d-flex flex-row justify-content-center gap-1 pt-4 no-print-btn" data-html2canvas-ignore="true">
                                    <button type="button" class="btn btn-success btn-icon btn-sm rounded-circle approveSign" data-role="finance" data-action="approve" title="Approve"><i class="ti ti-check fs-6"></i></button>
                                    <button type="button" class="btn btn-danger btn-icon btn-sm rounded-circle approveSign" data-role="finance" data-action="reject" title="Reject"><i class="ti ti-x fs-6"></i></button>
                                    <button type="button" class="btn btn-warning btn-icon btn-sm rounded-circle approveSign" data-role="finance" data-action="revision" title="Revision"><i class="ti ti-refresh fs-6"></i></button>
                                </div>
                                <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                            </div>
                            <div class="mt-2 fw-bold small <?php echo e($s5 ? 'text-uppercase' : ''); ?>" style="font-size: 14px;"><?php echo e($s5?->user?->name ?? 'Finance Staff'); ?></div>
                        </div>
                        ...
                        <div class="col-12 col-sm-6 col-md-4 mb-4">
                            ...
                            <div class="border-bottom mx-auto position-relative image-container" style="height: 100px; width: 120px; border-color: #333 !important;">
                                ...
                                <?php
                                $qrText6 = "SR: " . ($sr->pr_number ?? 'DRAFT') . "\nVerified By: " . ($s6->user->name ?? '-') . "\nRole: FINANCE SUPERVISOR\nDate: " . $s6->created_at->format('d/m/Y H:i');
                                ?>
                                <div class="pt-2">
                                    <img src="<?php echo e($this->getQr($qrText6)); ?>" class="signature-img" style="height: 80px; width: 80px; object-fit: contain; display: block; margin: 0 auto;">
                                </div>
                                <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                                ...
                                <div class="d-flex flex-row justify-content-center gap-1 pt-4 no-print-btn" data-html2canvas-ignore="true">
                                    <button type="button" class="btn btn-success btn-icon btn-sm rounded-circle approveSign" data-role="spvfinance" data-action="approve" title="Approve"><i class="ti ti-check fs-6"></i></button>
                                    <button type="button" class="btn btn-danger btn-icon btn-sm rounded-circle approveSign" data-role="spvfinance" data-action="reject" title="Reject"><i class="ti ti-x fs-6"></i></button>
                                    <button type="button" class="btn btn-warning btn-icon btn-sm rounded-circle approveSign" data-role="spvfinance" data-action="revision" title="Revision"><i class="ti ti-refresh fs-6"></i></button>
                                </div>
                                <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                            </div>
                            <div class="mt-2 fw-bold small <?php echo e($s6 ? 'text-uppercase' : ''); ?>" style="font-size: 14px;"><?php echo e($s6?->user?->name ?? 'Finance Supervisor'); ?></div>
                        </div>

                        
                        <div class="col-12 col-sm-6 col-md-4 mb-4">
                            <div class="fw-bold mb-1 small text-uppercase" style="font-size: 14px;">Authorized By</div>
                            <div class="border-bottom mx-auto position-relative image-container" style="height: 100px; width: 120px; border-color: #333 !important;">
                                <?php $s7 = $sign(7); ?>
                                <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if($s7): ?>
                                <?php
                                $qrText7 = "SR: " . ($sr->pr_number ?? 'DRAFT') . "\nAuthorized By: " . ($s7->user->name ?? '-') . "\nRole: CHIEF FINANCE OFFICER\nDate: " . $s7->created_at->format('d/m/Y H:i');
                                ?>
                                <div class="pt-2">
                                    <img src="<?php echo e($this->getQr($qrText7)); ?>" class="signature-img" style="height: 80px; width: 80px; object-fit: contain; display: block; margin: 0 auto;">
                                </div>
                                <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                                ...
                                <div class="d-flex flex-row justify-content-center gap-1 pt-4 no-print-btn" data-html2canvas-ignore="true">
                                    <button type="button" class="btn btn-success btn-icon btn-sm rounded-circle approveSign" data-role="cfo" data-action="approve" title="Approve"><i class="ti ti-check fs-6"></i></button>
                                    <button type="button" class="btn btn-danger btn-icon btn-sm rounded-circle approveSign" data-role="cfo" data-action="reject" title="Reject"><i class="ti ti-x fs-6"></i></button>
                                    <button type="button" class="btn btn-warning btn-icon btn-sm rounded-circle approveSign" data-role="cfo" data-action="revision" title="Revision"><i class="ti ti-refresh fs-6"></i></button>
                                </div>
                                <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>
                            </div>
                            <div class="mt-2 fw-bold small <?php echo e($s7 ? 'text-uppercase' : ''); ?>" style="font-size: 14px;"><?php echo e($s7?->user?->name ?? 'Chief Finance Officer'); ?></div>
                        </div>
                    ...
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            ...
                            <div id="revisionBox" class="d-none mb-3">
                                ...
                                <textarea class="form-control border-warning" id="revision_reason" rows="3" placeholder="Explain what needs to be fixed..."></textarea>
                            </div>
                            <div id="rejectBox" class="d-none mb-3">
                                <label class="form-label fw-bold">Reject Reason <span class="text-danger">*</span></label>
                                <textarea class="form-control border-danger" id="reject_reason" rows="3" placeholder="Explain why it's rejected..."></textarea>
                            </div>
                        </div>
                        <div class="modal-footer">
                            <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>
                            <button type="button" id="btnApprove" class="btn btn-success d-none">Confirm Approve</button>
                            <button type="button" id="btnSubmitPrConfirm" class="btn btn-primary d-none">Confirm Submit SR</button>
                            <button type="button" id="btnSubmitRevision" class="btn btn-warning d-none">Send for Revision</button>
                            <button type="button" id="btnSubmitReject" class="btn btn-danger d-none">Confirm Reject</button>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="table-responsive dt-responsive">
                <table class="table table-striped table-bordered nowrap">
                    ...
                    <tbody>
                        <?php $no = 1; ?>
                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__currentLoopData = $sr->signTransactions->whereIn('status', [12,13]); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $t): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoop($loop->index); ?><?php endif; ?>
                        <tr>
                            <td class="text-center"><?php echo e($no++); ?></td>
            ...
            <?php $dirReason = $sr->signTransactions->whereNotNull('director_reason')->first(); ?>
            ...
            <div class="table-responsive dt-responsive">
                <table class="table table-striped table-bordered nowrap">
                    ...
                    <tbody>
                        <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::openLoop(); ?><?php endif; ?><?php $__currentLoopData = $sr->signTransactions->whereNotNull('director_reason'); $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $idx => $t): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><?php \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::startLoop($loop->index); ?><?php endif; ?>
                        <tr>
...
[$__name, $__params] = $__split('settlement-reports.form-modal');

$key = null;
$__componentSlots = [];

$key ??= \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::generateKey('lw-645546955-6', $key);
...
[$__name, $__params] = $__split('settlement-reports.form-detail-modal', ['srId' => $sr->id_sr]);

$key = null;
$__componentSlots = [];

$key ??= \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::generateKey('lw-645546955-7', $key);
...
[$__name, $__params] = $__split('settlement-reports.payment-modal', ['srId' => $sr->id_sr]);

$key = null;
$__componentSlots = [];

$key ??= \Livewire\Features\SupportCompiledWireKeys\SupportCompiledWireKeys::generateKey('lw-645546955-8', $key);
...
<script>
    ...
    function resetApproveModal() {
        ...
        ['approveBox', 'revisionBox', 'rejectBox', 'btnApprove', 'btnSubmitPrConfirm', 'btnSubmitRevision', 'btnSubmitReject'].forEach(id => {
    ...
    window.openApprovalModal = function(action, role = '') {
        ...
        if (action === 'submit') {
            document.getElementById('modalApproveTitle').innerText = 'Submit SR';
            ...
            document.getElementById('modalApproveTitle').innerText = 'Approve SR';
            document.getElementById('approveBox').classList.remove('d-none');
            ...
            document.getElementById('modalApproveTitle').innerText = 'Reject SR';
    ...
    document.getElementById('btnSubmitPrConfirm')?.addEventListener('click', function() {
        ...
        approveModal.hide();
        window.Livewire.find('<?php echo e($_instance->getId()); ?>').submitSr(note);
    });
    ...
    window.downloadPDF = function() {
        ...
        const options = {
            margin: 0,
            filename: 'PR-<?php echo e($sr->pr_number ?? "DRAFT"); ?>.pdf',
            image: {
                type: 'jpeg',
                quality: 0.85
            },
...
<?php $__env->stopPush(); ?><?php /**PATH D:\!Kerja\laracok - Copy\resources\views\livewire\settlement-reports\show.blade.php ENDPATH**/ ?>