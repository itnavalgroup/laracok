                ...
                <div class="col-md-3">
                    <div class="card summary-card bg-primary h-100">
                        <div class="card-body">
                            ...
                            <h2 class="mb-0 fw-bold text-white">{{ $totalPositions }}</h2>
                        </div>
                    </div>
                </div>
                <div class="col-md-9">
                    <div class="card border-0 shadow-sm h-100">
                        <div class="card-body d-flex align-items-center justify-content-between py-2">
                            ...
                            @if(auth()->user()->level === 1 || auth()->user()->hasPermission('position.create'))
            ...
            <div class="table-responsive shadow-sm rounded-3">
                <table class="modern-table mb-0">
                    ...
                    <tbody>
                        ...
                        <tr wire:key="position-{{ $p->id_position }}">
                            <td class="text-center text-muted small">{{ ($positions->currentPage()-1) * $positions->perPage() + $loop->iteration }}</td>
                            <td>
                                <div class="fw-bold modern-text-title text-uppercase">{{ $p->position }}</div>
                            </td>
                            <td class="text-center">
                                <div class="d-flex justify-content-center gap-2">
                                    @if(auth()->user()->level === 1 || auth()->user()->hasPermission('position.edit'))
                                    <button wire:click="edit({{ $p->id_position }})" class="btn btn-icon bg-light-warning rounded-circle" title="Edit">
                                        <i class="ti ti-edit fs-5"></i>
                                    </button>
                                    @endif

                                    @if(auth()->user()->level === 1 || auth()->user()->hasPermission('position.delete'))
                                    <button type="button"
                                        onclick="showConfirm({
                                                title: 'Hapus Posisi',
                                                message: 'Apakah Anda yakin ingin menghapus posisi {{ $p->position }}? Tindakan ini tidak dapat dibatalkan.',
                                                type: 'danger',
                                                onConfirm: () => @this.delete({{ $p->id_position }})
                                            })"
                ...
                <form wire:submit.prevent="save">
                    ...
                    <div class="modal-footer border-0">
                        ...
                        <button type="submit" class="btn btn-primary rounded-pill px-4" wire:loading.attr="disabled">
                            <i class="ti ti-device-floppy me-2"></i> {{ $isEditing ? 'Update Data' : 'Save Position' }}
                        </button>
                    </div>
                </form>
            </div>
