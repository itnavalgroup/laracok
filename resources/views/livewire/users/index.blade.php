                ...
                <div class="col-md-3">
                    <div class="card summary-card bg-info">
                        <div class="card-body">
                            ...
                            <h2 class="mb-0 fw-bold">{{ $perPage }}</h2>
                        </div>
                    </div>
                </div>
                <div class="col-md-3">
                    <div class="card summary-card bg-success">
                        <div class="card-body">
                            ...
                            <h2 class="mb-0 fw-bold">{{ $currentPage }}</h2>
                        </div>
                    </div>
                </div>
                <div class="col-md-3">
                        ...
                        <tr>
                            ...
                            <td>
                                <div class="avatar avatar-sm bg-primary-subtle rounded-circle d-flex align-items-center justify-content-center overflow-hidden" style="width: 45px; height: 45px;">
                                    @if($user->photo)
                                    <img src="{{ asset('storage/image/'.$user->photo) }}" class="img-fluid h-100 w-100 object-fit-cover" alt="User">
                                    @else
                            ...
                            <td>
                                <div class="fw-bold modern-text-title">{{ $user->name }}</div>
                                <div class="modern-text-muted small">ID: {{ $user->id_employee }}</div>
                                <span class="badge bg-light text-primary border border-primary-subtle mt-1" style="font-size: 0.65rem;">
                                    {{ $user->level_detail->level_name ?? '-' }}
                                </span>
                            </td>
                            <td>{{ $user->position->position ?? '-' }}</td>
                            <td>{{ $user->departement->departement ?? '-' }}</td>
                            <td>
                                <div class="small modern-text-muted"><i class="ti ti-phone me-1 opacity-50"></i>{{ $user->phone ?? '-' }}</div>
                            </td>
                            <td>
                                <div class="small modern-text-muted text-truncate" style="max-width: 150px;" title="{{ $user->primary_email->email ?? '-' }}">
                                    <i class="ti ti-mail me-1 opacity-50"></i>{{ $user->primary_email->email ?? '-' }}
                                </div>
                            </td>
                            <td>
                                <span class="badge {{ $user->is_active ? 'bg-success-subtle text-success' : 'bg-danger-subtle text-danger' }} rounded-pill px-3">
                                    {{ $user->is_active ? 'Active' : 'Inactive' }}
                                </span>
                            </td>
                            <td class="text-end">
                                <div class="d-flex justify-content-end gap-1">
                                    <a href="{{ route('users.show', hashid_encode($user->id_user)) }}" class="btn btn-sm btn-icon btn-outline-info rounded-circle border-0" title="View Details">
                                        <i class="ti ti-eye fs-5"></i>
                                    </a>

                                    @if(auth()->user()->canEditUser($user))
                                    <a href="{{ route('users.edit', hashid_encode($user->id_user)) }}" class="btn btn-sm btn-icon btn-outline-primary rounded-circle border-0" title="Edit">
                                    ...
                                    <button type="button"
                                        onclick="showConfirm({
                                            ...
                                            type: 'danger',
                                            onConfirm: () => @this.delete({{ $user->id_user }})
                                        })"
