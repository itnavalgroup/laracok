                ...
                <div class="col-md-3">
                    <div class="card summary-card bg-primary h-100">
                        <div class="card-body">
                            ...
                            <h2 class="mb-0 fw-bold text-white">{{ $totalDocTypes }}</h2>
                        </div>
                    </div>
                </div>
                <div class="col-md-9">
                    <div class="card border-0 shadow-sm h-100">
                        <div class="card-body d-flex align-items-center justify-content-between py-2">
                            ...
                            @if(auth()->user()->level === 1 || auth()->user()->hasPermission('doc_type.create'))
            ...
            <div class="table-responsive shadow-sm rounded-3">
                <table class="modern-table mb-0">
                    ...
                    <tbody>
                        ...
                        <tr wire:key="doc-type-{{ $dt->id_doc_type }}">
                            <td class="text-center text-muted small">{{ ($docTypes->currentPage()-1) * $docTypes->perPage() + $loop->iteration }}</td>
                            <td>
                                <div class="fw-bold modern-text-title text-uppercase">{{ $dt->doc_type }}</div>
                            </td>
                            <td class="text-center">
                                <div class="d-flex justify-content-center gap-2">
                                    @if(auth()->user()->level === 1 || auth()->user()->hasPermission('doc_type.edit'))
                                    <button wire:click="edit({{ $dt->id_doc_type }})" class="btn btn-icon bg-light-warning rounded-circle" title="Edit">
                                        <i class="ti ti-edit fs-5"></i>
                                    </button>
                                    @endif

                                    @if(auth()->user()->level === 1 || auth()->user()->hasPermission('doc_type.delete'))
                                    <button type="button"
                                        onclick="showConfirm({
                                                title: 'Hapus Tipe Dokumen',
                                                message: 'Apakah Anda yakin ingin menghapus tipe dokumen {{ $dt->doc_type }}? Tindakan ini tidak dapat dibatalkan.',
                                                type: 'danger',
                                                onConfirm: () => @this.delete({{ $dt->id_doc_type }})
                                            })"
            ...
            <div class="modal-content border-0 shadow-lg">
                <div class="modal-header bg-primary text-white py-3">
                    <h5 class="modal-title fw-bold text-white text-uppercase" id="docTypeModalLabel">
                        ...
                        {{ $isEditing ? 'Edit Tipe Dokumen' : 'Tambah Tipe Dokumen' }}
                    </h5>
                ...
                <form wire:submit.prevent="save">
                    <div class="modal-body p-4">
                        <div class="row g-3">
                            ...
                            <div class="col-12">
                                ...
                                <input type="text" class="form-control @error('doc_type') is-invalid @enderror"
                    ...
                    <div class="modal-footer border-0">
                        ...
                        <button type="submit" class="btn btn-primary rounded-pill px-4" wire:loading.attr="disabled">
                            <i class="ti ti-device-floppy me-2"></i> {{ $isEditing ? 'Update Data' : 'Simpan Tipe Dokumen' }}
                        </button>
                    </div>
                </form>
            </div>
