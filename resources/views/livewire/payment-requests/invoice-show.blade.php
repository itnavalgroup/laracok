<div>
            ...
            <div class="invoice-card card">
                ...
                <div class="card-body" style="padding: 2rem;">
                    ...
                    <div class="invoice-summary mb-4">
                        ...
                        <p class="fs-4 fw-bold mb-1"><i class="fas fa-file-invoice-dollar me-2" style="font-size:1.5rem"></i>{{ $invoice->invoice_number }}</p>
                        <small>Date: {{ $invoice->invoice_date ? $invoice->invoice_date->format('d M Y') : '-' }} | Delivery: {{ $invoice->delivery_date ? $invoice->delivery_date->format('d M Y') : '-' }}</small>
                    </div>
                        ...
                        <div class="row g-4">
                            <div class="col-md-6">
                                <div class="mb-3">
                                    ...
                                    <div class="fw-bold fs-6">{{ $pr->company->company ?? '-' }}</div>
                                </div>
                                <div class="mb-3">
                                    <div class="text-muted fw-semibold" style="font-size:0.85rem">TRUCK</div>
                                    <div class="fw-bold fs-6">{{ $invoice->truck ?: '-' }}</div>
                                </div>
                            </div>
                            <div class="col-md-6">
                                <div class="mb-3">
                                    <div class="text-muted fw-semibold" style="font-size:0.85rem">INVOICE DATE</div>
                                    <div class="fw-bold fs-6">{{ $invoice->invoice_date ? $invoice->invoice_date->format('d M Y') : '-' }}</div>
                                </div>
                                <div class="mb-3">
                                ...
                                <div class="mb-3">
                                    ...
                                    <div class="fw-bold fs-6">{{ $invoice->delivery_date ? $invoice->delivery_date->format('d M Y') : '-' }}</div>
                                </div>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="table-responsive">
                            <table class="modern-table mb-0">
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        <td>{{ $i + 1 }}</td>
                                        <td class="text-wrap" style="max-width:300px">{{ $item->detail }}</td>
                                        <td class="text-center">{{ number_format($item->qty, 2, ',', '.') }}</td>
                                        <td class="text-center">{{ $item->uom->uom ?? '-' }}</td>
                                        <td class="text-end">{{ $symbol }} {{ number_format($item->price, 2, ',', '.') }}</td>
                                        <td class="text-end">{{ $symbol }} {{ number_format($item->discount, 2, ',', '.') }}</td>
                                        <td class="text-end">{{ $symbol }} {{ number_format($item->tax1, 2, ',', '.') }}</td>
                                        <td class="text-end">{{ $symbol }} {{ number_format($item->tax2, 2, ',', '.') }}</td>
                                        <td class="fw-bold text-end">{{ $symbol }} {{ number_format($item->ammount, 2, ',', '.') }}</td>
                                    </tr>
                                    @endforeach
                                </tbody>
                                <tfoot>
                                    <tr>
                                        ...
                                        <th class="text-end bg-light">{{ $symbol }} {{ number_format($discount, 2, ',', '.') }}</th>
                                    </tr>
                                    <tr>
                                        <th colspan="8" class="text-end" style="background-color: #e8f5e8;">TOTAL PAYMENT</th>
                                        <th class="text-end fw-bold text-primary bg-light fs-6">{{ $symbol }} {{ number_format($grandTotal, 2, ',', '.') }}</th>
                                    </tr>
                                </tfoot>
                            </table>
                        </div>
                    ...
                    <div class="signature-section border-0 p-0 m-0">
                        <div class="row text-center mt-4">
                            ...
                            <div class="col-md-6 mb-4">
                                <div class="fw-bold mb-5 pb-3">{{ $pr->vendor->vendor ?? 'VENDOR' }}</div>
                                <div class="border-bottom mx-auto" style="width: 200px; border-color: #000 !important;"></div>
                                <div class="fw-bold mt-2">NAME :</div>
                            </div>
                        </div>
                    ...
                    <div class="attachment-section border-top pt-4 mb-4">
                        <div class="d-flex justify-content-between align-items-center mb-4">
                            ...
                            @if(empty($invoice->file_name) && $canUploadAttachment)
                            ...
                            <div class="col-md-12">
                                ...
                                <div class="attachment-item shadow-sm border">
                                    ...
                                    <div class="flex-grow-1 ms-3">
                                        <a href="#" onclick="openInvoicePreview('{{ asset('assets/invoice/' . $invoice->file_name) }}'); return false;" class="text-decoration-none fw-bold fs-6 text-dark" style="transition:all 0.2s;">
                                            {{ $invoice->invoice_number ?: '-' }}
                                        </a>
                                        <div class="text-muted" style="font-size:0.875rem">{{ Str::upper($ext) }} Document</div>
                                    </div>
                    ...
                    <div class="d-flex justify-content-end gap-2 pt-4 border-top">
                        ...
                        <button type="button" class="btn btn-danger" onclick="showConfirm({
                            ...
                            btnText: 'Ya, Hapus',
                            onConfirm: () => {
                                window.location.href = '{{ route('payment-requests.invoice.delete', hashid_encode($invoice->id_invoice, 'invoice')) }}';
                            }
                        })">
                        ...
                        @if($canPrint)
                        <button id="btnPrintInvoice" onclick="silentAction('{{ route('payment-requests.invoice.print', $prHash) }}', 'print', this)" class="btn btn-info text-white">
                            <i class="fas fa-print me-1"></i> Print
                        </button>
                        @endif

                        @if($canDownload)
                        <button id="btnDownloadInvoice" onclick="silentAction('{{ route('payment-requests.invoice.print', $prHash) }}', 'download', this)" class="btn btn-success">
                            <i class="fas fa-file-pdf me-1"></i> Download PDF
                        </button>
                        @endif
                    </div>

                    {{-- Hidden Iframe for Silent Print/Download --}}
    ...
    <div class="modal fade" id="modalCreateInvoice" tabindex="-1" aria-hidden="true" wire:ignore.self>
        <div class="modal-dialog modal-md modal-dialog-centered">
            <div class="modal-content">
                <form method="POST" action="{{ route('payment-requests.invoice.store', $prHash) }}">
                    @csrf
    ...
    <div class="modal fade" id="modalEditInvoice" tabindex="-1" aria-hidden="true" wire:ignore.self>
        <div class="modal-dialog modal-md modal-dialog-centered">
            <div class="modal-content">
                <form method="POST" action="{{ route('payment-requests.invoice.update', hashid_encode($invoice->id_invoice, 'invoice')) }}">
                    @csrf
                    <div class="modal-header" style="background: linear-gradient(135deg,#667eea,#764ba2); color:#fff;">
                    ...
                    <div class="modal-body">
                        <div class="mb-3">
                            ...
                            <input type="text" name="truck" class="form-control" value="{{ $invoice->truck }}" autocomplete="off">
                        </div>
                        <div class="mb-3">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="date" name="delivery_date" class="form-control" value="{{ $invoice->delivery_date?->format('Y-m-d') }}">
                        </div>
                        @if($pr->no_invoice != $invoice->invoice_number)
                        <div class="alert alert-warning mb-0 p-3 mt-4" style="border-radius:8px;">
                            <div class="form-check mb-0">
                            ...
                            <small class="text-danger d-block mt-1">Jika dicentang, nomor invoice PR ({{ $pr->no_invoice ?: '-' }}) akan diperbarui menjadi {{ $invoice->invoice_number }}.</small>
                        </div>
                        @endif
                    </div>
                    <div class="modal-footer">
    ...
    <div class="modal fade" id="modalAddAttInvoice" tabindex="-1" aria-hidden="true" wire:ignore.self>
        <div class="modal-dialog modal-lg modal-dialog-centered">
            <div class="modal-content">
                <form id="formAddAttInvoice" method="POST" action="{{ route('payment-requests.invoice.attachment.store', hashid_encode($invoice->id_invoice, 'invoice')) }}" enctype="multipart/form-data">
                    @csrf
    ...
    <div class="modal fade" id="modalEditAttInvoice" tabindex="-1" aria-hidden="true" wire:ignore.self>
        <div class="modal-dialog modal-lg modal-dialog-centered">
            <div class="modal-content">
                <form id="formEditAttInvoice" method="POST" action="{{ route('payment-requests.invoice.attachment.update', hashid_encode($invoice->id_invoice, 'invoice')) }}" enctype="multipart/form-data">
                    @csrf
            ...
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" class="btn btn-danger" onclick="showConfirm({
                        ...
                        btnText: 'Ya, Hapus',
                        onConfirm: () => {
                            window.location.href = '{{ route('payment-requests.invoice.attachment.delete', hashid_encode($invoice->id_invoice, 'invoice')) }}';
                        }
                    })">
                        <i class="fas fa-trash me-1"></i> Delete
                    </button>
                    @endif
                    <a id="btnDownloadPreview" href="{{ asset('assets/invoice/' . $invoice->file_name) }}" class="btn btn-success" download>
