<div class="modal fade" id="ikbDetailModal" tabindex="-1" aria-labelledby="ikbDetailModalLabel" aria-hidden="true" wire:ignore.self>
    <div class="modal-dialog modal-dialog-centered modal-lg">
        <div class="modal-content border-0 shadow-lg" style="border-radius: 12px;"
            x-data="ikbDetailData()"
            x-init="initForm()">
            ...
            <div class="modal-body p-4">
                ...
                <div class="row g-3">
                        ...
                        <select x-model="form.id_item_category" class="form-select select2-ikb-detail" id="ikb_detail_category"
                            {{ $isInvCtrlEditMode ?? false ? 'disabled' : '' }}>
                    ...
                    <div class="col-md-4">
                        <label class="fw-bold small text-uppercase">Quantity Request <span class="text-danger">*</span></label>
                        ...
                        <span class="text-danger small" x-show="errors.qty" x-text="errors.qty"></span>
                                        ...
                                        <div class="col-md-4">
                        <label class="fw-bold small text-uppercase">UOM Request <span class="text-danger">*</span></label>
                        ...
                        <select x-model="form.id_packaging" class="form-select select2-ikb-detail" id="ikb_detail_packaging"
                            {{ $isInvCtrlEditMode ?? false ? 'disabled' : '' }}>
                        ...
                        <span class="text-danger small" x-show="errors.id_packaging" x-text="errors.id_packaging"></span>
                                        ...
                                        @if($canViewContract)
                    <div class="col-md-12">
                        <label class="fw-bold small text-uppercase">Contract <span class="text-muted fw-normal">(Opsional)</span></label>
                        <select x-model="form.id_contract" class="form-select select2-ikb-detail" id="ikb_detail_contract">
                            <option value="">-- Tanpa Contract --</option>
                            @foreach($contracts as $contract)
                            <option value="{{ $contract->id_contract }}">{{ $contract->contract_number }}</option>
                            @endforeach
                        </select>
                    </div>
                    @endif
...
<script>
    function ikbDetailData() {
        return {
            ...
            errors: {},
            allItems: @json($items),
            isInvCtrlEditMode: {{ $isInvCtrlEditMode ? 'true' : 'false' }},
                ...
                if (data.detail) {
                    ...
                    Object.keys(this.form).forEach(k => {
                        ...
                        if (k === 'qty' && val !== null) {
                            ...
                            this.form[k] = parseFloat(val).toLocaleString('en-US', { minimumFractionDigits: 0, maximumFractionDigits: 2 });
                        } else {
            ...
            async submitForm() {
                ...
                if (this.form.id_item && this.form.qty) {
                    ...
                    if (selectedItem) {
                        ...
                        if (!this.isEditing && reqQty > availStock) {
                            this.errors.qty = 'Quantity melampaui stok tersedia (' + availStock.toLocaleString('en-US') + ')';
                ...
                this.saving = true;
                try {
                    const result = await @this.saveFromJs(this.form, this.detailId);
            ...
            initSelect2() {
                ...
                if (this.isInvCtrlEditMode) {
                    $('#ikb_detail_category, #ikb_detail_item, #ikb_detail_uom, #ikb_detail_packaging, #ikb_detail_contract').prop('disabled', true);
                    // Force select2 to update its disabled visual state
                    $('#ikb_detail_category, #ikb_detail_item, #ikb_detail_uom, #ikb_detail_packaging, #ikb_detail_contract').trigger('change.select2');
                } else {
                    $('#ikb_detail_category, #ikb_detail_item, #ikb_detail_uom, #ikb_detail_packaging, #ikb_detail_contract').prop('disabled', false);
                    $('#ikb_detail_category, #ikb_detail_item, #ikb_detail_uom, #ikb_detail_packaging, #ikb_detail_contract').trigger('change.select2');
                }
            },
            ...
            updateItemOptions() {
                ...
                this.filteredItems().forEach(item => {
                    ...
                    const stockDisplay = parseFloat(item.available_stock).toLocaleString('en-US', { minimumFractionDigits: 2 });
                    $itemSelect.append(`<option value="${item.id_item}" ${selected}>${item.item_code} - ${item.item_name} (Stock: ${stockDisplay})</option>`);
                });
