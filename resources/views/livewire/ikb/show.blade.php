<div class="ikb-detail">
    ...
    <template x-teleport="#ikb-header-actions">
        <div class="d-flex align-items-center gap-2">
            @if($canSubmit)
                <button type="button" class="btn btn-primary btn-sm rounded-pill shadow-sm px-3 fw-bold" onclick="openApprovalModal('approve', {{ $status }})">
                    <i class="ti ti-send me-1"></i> Submit
                </button>
            @endif

            @if($canApproveCurrent)
                <button type="button" class="btn btn-success btn-sm rounded-pill shadow-sm px-3 fw-bold" onclick="openApprovalModal('approve', {{ $status }})">
                    <i class="ti ti-check me-1"></i> Approve
                </button>
                <button type="button" class="btn btn-warning btn-sm rounded-pill shadow-sm px-3 fw-bold" onclick="openApprovalModal('revision', {{ $status }})">
                    <i class="ti ti-refresh me-1"></i> Revision
                </button>
                <button type="button" class="btn btn-danger btn-sm rounded-pill shadow-sm px-3 fw-bold" onclick="openApprovalModal('reject', {{ $status }})">
                    <i class="ti ti-x me-1"></i> Reject
                </button>
            @endif

            @if($canCancelCurrent)
                <button type="button" class="btn btn-outline-danger btn-sm rounded-pill shadow-sm px-3 fw-bold bg-white" onclick="showCancelModal('cancelApproval')">
    ...
    <div class="row">
        <div class="col-12">
            <div class="card p-3 border-0 shadow-sm">
                ...
                <div class="d-flex justify-content-between align-items-start mb-4 border-bottom pb-3">
                    ...
                    <div class="d-flex align-items-center gap-3">
                        ...
                        <div class="d-flex gap-2">
                            ...
                            @if($canDownload)
                            <button id="btnDownloadPDF" onclick="downloadPDF()" class="btn btn-danger btn-sm no-print-btn">
                            ...
                            @endif
                            @if($canPrint)
                            <button onclick="window.printIKB()" class="btn btn-primary btn-sm no-print-btn">Print</button>
                            @endif
                            @if($canEditHeader)
                            <button type="button" wire:click="$dispatch('open-ikb-form', { id: {{ $ikb->id_ikb }} })" class="btn btn-warning btn-sm no-print-btn">Edit</button>
                            @endif
                        </div>
                        {{-- QR Code --}}
                        @if($ikb->qr)
                        <img alt="QR IKB" src="{{ asset('assets/qr/' . $ikb->qr) }}" style="height: 60px; width: 60px; object-fit: contain;">
                        @endif
                    </div>
                </div>
                ...
                {{-- Status Info --}}
                <div class="alert alert-{{ $sbadge['color'] }} d-flex align-items-center mb-4" role="alert" style="padding: 8px 15px; border-radius: 4px;">
                    <div style="text-decoration: underline; font-weight:600; font-size: 13px;">IKB DETAIL INFORMATION</div>
                    <div class="mx-3">
                        <span class="badge bg-{{ $sbadge['color'] }}" style="font-size:11px; padding: 4px 8px;">{{ $sbadge['label'] }}</span>
                    </div>
                </div>
                ...
                <div class="row g-3" style="font-size: 15px;">
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="row mb-2">
                            ...
                            <div class="col-1 text-center">:</div>
                            <div class="col-7">{{ $ikb->booking_date ? $ikb->booking_date->format('d F Y') : '-' }}</div>
                        </div>
                        <div class="row mb-2">
                            ...
                            <div class="col-7 d-flex align-items-center gap-2">
                                ...
                                @elseif($canApproveStep8 && $status == 8)
                                    <button class="btn btn-primary btn-sm no-print-btn"
                                        ...
                                        data-bs-toggle="modal" data-bs-target="#modalDeliveryDate">
                                        📅 Set Delivery Date
                                    </button>
                                @else
                                    -
                        ...
                        <div class="row mb-2">
                            ...
                            <div class="col-7 d-flex align-items-center gap-2">
                                ...
                                @elseif($canApproveStep6 && $status == 6)
                                    <button class="btn btn-primary btn-sm no-print-btn"
                                        style="font-size:11px;padding:2px 8px;"
                                        data-bs-toggle="modal" data-bs-target="#modalStuffingDate">
                                        📅 Set Stuffing Date
                                    </button>
                                @else
                                    -
                ...
                <div class="row g-3" style="font-size: 15px;">
                    ...
                    <div class="col-md-6">
                        ...
                        <div class="row mb-2">
                            <div class="col-4 fw-bold text-uppercase">VENDOR</div>
                            <div class="col-1 text-center">:</div>
                ...
                <div class="card-header align-items-center bg-transparent d-flex px-0 py-3 mt-4 border-bottom">
                    ...
                    <div class="d-flex gap-2">
                        ...
                        <button class="btn btn-primary btn-sm no-print-btn" wire:click="$dispatchTo('ikb.form-detail-modal', 'openModal')">ADD ITEM</button>
                        @endif
                    </div>
                </div>
                <div class="table-responsive mt-2">
                    <table class="table table-bordered table-striped" style="font-size: 15px;">
                            ...
                            <tr>
                                ...
                                <th>UOM</th>
                                <th>PACKAGING</th>
                                @if($status == 0 || $status == 11 || ($status == 4 && $canEditDetail))<th style="width: 80px;" class="no-print-btn">AKSI</th>@endif
                            </tr>
                        </thead>
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="text-center fw-bold">{{ number_format($detail->qty, 2, '.', ',') }}</td>
                            ...
                            @empty
                            <tr>
                                <td colspan="8" class="text-center py-4 text-muted small">Belum ada item ditambahkan.</td>
                            </tr>
                            @endforelse
                        </tbody>
                    </table>
                ...
                <div class="card-header align-items-center bg-transparent d-flex px-0 py-3 mt-4 border-bottom">
                    ...
                    <div class="flex-shrink-0">
                        @php
                        $canViewAtt = $isAdmin || $isOwner || ($user->id_user == $ikb->sales) || $user->hasPermission('ikb.view.attachment');
                        @endphp
                        @if($status < 9 && ($isAdmin || (($status==0 || $status==11) && $isOwner && $user->hasPermission('ikb_detail.create')) || ($status == 8 && $user->hasPermission('ikb.approve.step8'))))
                            <button class="btn btn-primary no-print-btn" data-bs-toggle="modal" data-bs-target="#modalAddAttachment">ADD</button>
                            @endif
                    </div>
                </div>
                <div class="row g-3" style="font-size: 15px;">
                    ...
                    $lastSignStep7 = null;
                    if ($status == 8) {
                    $lastSignStep7 = \App\Models\SignTransaction::where('id_ikb', '=', $ikb->id_ikb)->where('status', '=', 7)->latest()->first();
                    }
                    @endphp
                    ...
                    if ($isOwner && ($user->hasPermission('ikb_detail.edit') || $user->hasPermission('ikb_detail.delete'))) {
                    $canEditAtt = true;
                    }
                    } elseif ($status == 8) {
                    // In Step 8, Security can only edit what they themselves uploaded during this step
                    if ($lastSignStep7 && $att->created_at > $lastSignStep7->created_at && $att->id_user == $user->id_user) {
                    $canEditAtt = true;
                    }
                    }
                    ...
                    <div class="col-md-2 d-flex align-items-center mb-3">
                        <div class="form-check form-check-inline d-flex align-items-center">
                            <input type="checkbox" class="form-check-input input-primary me-2" checked onclick="return false;">
                    ...
                    <div class="row g-3 text-center">
                        ...
                        <div class="col">
                            ...
                            <div class="border-bottom mx-auto position-relative image-container" style="height: 100px; width: 120px; border-color: #333 !important;">
                                ...
                                @if($s0)
                                @php
                                $qrText = "IKB: " . ($ikb->ikb_number ?? 'DRAFT') . "\nBy: " . ($s0->user->name ?? '-') . "\nDate: " . $s0->created_at->format('d/m/Y H:i') . "\nUrl: " . route('ikb.show', hashid_encode($ikb->id_ikb, 'ikb'));
                                @endphp
                                <div class="pt-2">
                                    <img src="{{ $this->getQr($qrText) }}" style="height: 80px; width: 80px; object-fit: contain;">
                                </div>
                                @if($status == 1 && ($isAdmin || ($isSales && $user->hasPermission('ikb.cancel_submit'))))
                                <button type="button" class="btn btn-outline-danger btn-xs position-absolute top-50 start-50 translate-middle no-print-btn"
                                    onclick="showCancelModal('cancelSubmit')">Cancel</button>
                                @endif
                                @elseif($canSubmit)
                                <button onclick="openApprovalModal('approve', 0)" class="btn btn-outline-primary btn-xs mt-4 no-print-btn" style="font-size: 12px;">SUBMIT</button>
                                @endif
                            </div>
                            <div class="mt-2 fw-bold small text-uppercase" style="font-size: 14px;">{{ $s0->user->name ?? $ikb->salesUser->name ?? '-' }}</div>
                        ...
                        @foreach($cols as $col)
                        <div class="col">
                            <div class="fw-bold mb-1 small text-uppercase" style="font-size: 14px;">{{ $col['title'] }}</div>
                            <div class="border-bottom mx-auto position-relative image-container" style="height: 100px; width: 120px; border-color: #333 !important;">
                                @php $s = $sign($col['step']); @endphp
                                @if($s)
                                @php
                                $qrText = "IKB: " . ($ikb->ikb_number ?? 'DRAFT') . "\nApproved By: " . ($s->user->name ?? '-') . "\nRole: " . $col['role'] . "\nDate: " . $s->created_at->format('d/m/Y H:i') . "\nUrl: " . route('ikb.show', hashid_encode($ikb->id_ikb, 'ikb'));
                                @endphp
                                <div class="pt-2">
                                    <img src="{{ $this->getQr($qrText) }}" style="height: 80px; width: 80px; object-fit: contain;">
                                </div>
                                ...
                                if($col['step'] == 1) {
                                $subordinateIds = $user->subordinates()->pluck('id_user')->toArray();
                                if(!in_array($ikb->sales, $subordinateIds)) $canApprove = false;
                                }
                                }
                                @endphp
                                @if($canApprove)
                                <div class="d-flex flex-row justify-content-center gap-1 pt-4 px-1 no-print-btn" data-html2canvas-ignore="true">
                                    <button onclick="openApprovalModal('approve', {{ $col['step'] }})" class="btn btn-success btn-icon btn-sm" title="Approve"><i class="ti ti-check fs-6"></i></button>
                                    <button onclick="openApprovalModal('reject', {{ $col['step'] }})" class="btn btn-danger btn-icon btn-sm" title="Reject"><i class="ti ti-x fs-6"></i></button>
                                    <button onclick="openApprovalModal('revision', {{ $col['step'] }})" class="btn btn-warning btn-icon btn-sm" title="Revision"><i class="ti ti-refresh fs-6"></i></button>
                                </div>
                                @else
                                <span class="badge bg-light text-muted fw-normal mt-4 no-print-btn" style="font-size: 10px;" data-html2canvas-ignore="true">PENDING</span>
                                @endif
                                @endif
                            </div>
                            <div class="mt-2 fw-bold small text-uppercase" style="font-size: 14px;">{{ $s->user->name ?? '-' }}</div>
                            <div class="mt-1 x-small fw-bold">{{ $col['role'] }}</div>
                        </div>
                        @endforeach
                    </div>

                    <div class="alert alert-light border text-center fw-bold py-1 my-2 text-uppercase" style="font-size: 11px; letter-spacing: 1px; color: #555;">WAREHOUSE PROCESSING</div>
                    ...
                    <div class="row g-3 text-center justify-content-center">
                        ...
                        <div class="col-md-2">
                            ...
                            <div class="border-bottom mx-auto position-relative image-container" style="height: 100px; width: 120px; border-color: #333 !important;">
                                ...
                                @if($canApprove)
                                <div class="d-flex flex-row justify-content-center gap-1 pt-4 px-1 no-print-btn" style="z-index: 10;" data-html2canvas-ignore="true">
                                    <button onclick="openApprovalModal('approve', {{ $col['step'] }})" class="btn btn-success btn-icon btn-sm" title="Approve"><i class="ti ti-check fs-6"></i></button>
                                </div>
                                @else
                                <span class="badge bg-light text-muted fw-normal mt-4 no-print-btn" style="font-size: 10px;" data-html2canvas-ignore="true">PENDING</span>
                            ...
                            <div class="text-danger mt-1" style="font-size: 9px; font-weight: 600;">UPLOAD PHOTO ATTACHMENT FIRST</div>
                            @endif
                        </div>
                        @endforeach
                    </div>
            ...
            <div class="alert alert-danger d-flex align-items-center mb-2 no-print-history" role="alert" style="padding: 5px 15px;">
                <div style="text-decoration: underline; font-weight:600; font-size: 12px;">REJECT / REVISION HISTORY</div>
            </div>
            ...
            @php
            $approvalNotes = $ikb->signTransactions()
            ->where('id_doc_type', 4)
            ...
            <div class="alert alert-info d-flex align-items-center mb-2 no-print-history" role="alert" style="padding: 5px 15px;">
    ...
    <div class="modal fade" id="modalStuffingDate" tabindex="-1" aria-labelledby="modalStuffingDateLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-sm">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <label class="form-label fw-bold small text-uppercase">Stuffing Date <span class="text-danger">*</span></label>
                    <input type="date" wire:model="stuffingDate" class="form-control">
                    @error('stuffingDate') <div class="text-danger small mt-1">{{ $message }}</div> @enderror
                </div>
                <div class="modal-footer py-2 justify-content-between">
                    <button type="button" class="btn btn-light btn-sm" data-bs-dismiss="modal">Cancel</button>
                    <button type="button" class="btn btn-success btn-sm" wire:click="setStuffingDate" data-bs-dismiss="modal">
    ...
    <div class="modal fade" id="modalDeliveryDate" tabindex="-1" aria-labelledby="modalDeliveryDateLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-sm">
            <div class="modal-content">
                <div class="modal-header py-2">
                ...
                <div class="modal-body">
                    <label class="form-label fw-bold small text-uppercase">Delivery Date <span class="text-danger">*</span></label>
                    <input type="date" wire:model="deliveryDate" class="form-control">
                    @error('deliveryDate') <div class="text-danger small mt-1">{{ $message }}</div> @enderror
                </div>
                <div class="modal-footer py-2 justify-content-between">
                    <button type="button" class="btn btn-light btn-sm" data-bs-dismiss="modal">Cancel</button>
                    <button type="button" class="btn btn-success btn-sm" wire:click="setDeliveryDate" data-bs-dismiss="modal">
                        <i class="ti ti-check me-1"></i>Simpan
                    </button>
                </div>
            </div>
        ...
        <div class="modal-dialog modal-dialog-centered">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    ...
                    <button type="button" id="btnConfirmApprove" class="btn btn-success d-none">Confirm Approve</button>
                    <button type="button" id="btnConfirmRevision" class="btn btn-warning d-none">Send for Revision</button>
                    <button type="button" id="btnConfirmReject" class="btn btn-danger d-none">Confirm Reject</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="modal fade" id="modalAddAttachment" tabindex="-1" aria-hidden="true">
        <div class="modal-dialog modal-lg modal-dialog-centered">
            <div class="modal-content">
                <form id="formAddAttachment" method="POST" action="{{ route('ikb.attachment.store', hashid_encode($ikb->id_ikb, 'ikb')) }}" enctype="multipart/form-data">
                    @csrf
                    <div class="modal-header">
                ...
                <form id="formEditAttachment" method="POST" action="" enctype="multipart/form-data">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="file" class="form-control" name="filename" accept="image/*,application/pdf">
                        </div>
                    </div>
                    <div class="modal-footer">
                        <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Batal</button>
    ...
    <script>
        ...
        window.downloadPDF = function() {
            ...
            const options = {
                margin: 0,
                filename: 'IKB-{{ $ikb->ikb_number ?? "DRAFT" }}.pdf',
                image: {
                    type: 'jpeg',
                    quality: 0.85
                },
        ...
        window.openApprovalModal = function(action, step) {
            ...
            // Reset visibility
            ['approveBox', 'revisionBox', 'rejectBox', 'btnConfirmApprove', 'btnConfirmRevision', 'btnConfirmReject'].forEach(id => {
