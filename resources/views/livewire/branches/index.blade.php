                ...
                <div class="col-md-3">
                    <div class="card summary-card bg-primary h-100">
                        <div class="card-body">
                            ...
                            <h2 class="mb-0 fw-bold text-white">{{ $totalBranches }}</h2>
                        </div>
                    </div>
                </div>
                <div class="col-md-9">
                    <div class="card border-0 shadow-sm h-100">
                        <div class="card-body d-flex align-items-center justify-content-between py-2">
                            ...
                            @if(auth()->user()->level === 1 || auth()->user()->hasPermission('branch.create'))
            ...
            <div class="table-responsive shadow-sm rounded-3">
                <table class="modern-table mb-0">
                    ...
                    <tbody>
                        ...
                        <tr wire:key="branch-{{ $b->id_branch }}">
                            <td class="text-center text-muted small">{{ ($branches->currentPage()-1) * $branches->perPage() + $loop->iteration }}</td>
                            <td>
                                <div class="fw-bold modern-text-title">{{ $b->branch }}</div>
                            </td>
                            <td>
                            ...
                            <td class="text-center">
                                <div class="d-flex justify-content-center gap-2">
                                    @if(auth()->user()->level === 1 || auth()->user()->hasPermission('branch.edit'))
                                    <button wire:click="edit({{ $b->id_branch }})" class="btn btn-icon bg-light-warning rounded-circle" title="Edit">
                                        <i class="ti ti-edit fs-5"></i>
                                    </button>
                                    @endif

                                    @if(auth()->user()->level === 1 || auth()->user()->hasPermission('branch.delete'))
                                    <button wire:confirm="Are you sure you want to delete this branch? This action cannot be undone."
                                        wire:click="delete({{ $b->id_branch }})"
                ...
                <form wire:submit.prevent="save">
                    ...
                    <div class="modal-footer border-0">
                        ...
                        <button type="submit" class="btn btn-primary rounded-pill px-4" wire:loading.attr="disabled">
                            <i class="ti ti-device-floppy me-2"></i> {{ $isEditing ? 'Update Data' : 'Save Branch' }}
                        </button>
                    </div>
                </form>
            </div>
