                ...
                <div class="col-md-3">
                    <div class="card summary-card bg-primary h-100">
                        <div class="card-body">
                            ...
                            <h2 class="mb-0 fw-bold text-white">{{ $srs->total() }}</h2>
                        </div>
                    </div>
                </div>
                <div class="col-md-9">
                ...
                <div class="collapse mt-3 {{ $hasActiveFilter ? 'show' : '' }}" id="advancedTableFilters" wire:ignore.self>
                    ...
                    <div class="p-3 bg-light rounded-3 bg-opacity-50">
                        <div class="row g-3">
                            ...
                            @if(auth()->user()->level === 1 || auth()->user()->hasPermission('sr.view.all'))
                            <div class="col-md-4">
        ...
        <div class="col-12">
            <div class="table-responsive shadow-sm rounded-3">
                <table class="modern-table mb-0">
                    ...
                    <tbody>
                        @forelse($srs as $sr)
                        <tr wire:key="sr-{{ $sr->id_pr }}">
                            <td class="text-center text-muted small">{{ ($srs->currentPage()-1) * $srs->perPage() + $loop->iteration }}</td>
                            <td>
                                @php
                                $docTypes = [
                            ...
                            <td>
                                <a href="{{ route('settlement-reports.show', hashid_encode($sr->id_pr, 'pr')) }}" class="fw-bold text-primary text-decoration-none">
                                    {{ $sr->pr->pr_number ?? 'DRAFT' }}
                                </a>
                                <br>
                                @php
                                ...
                                <span class="text-muted small">PO: {{ $sr->pr->po_number ?? '-' }}</span>
                            </td>
                            <td>
                                <div class="fw-bold modern-text-title text-uppercase">{{ $sr->subject }}</div>
                                <div class="text-muted small mb-1">
                                    <i class="ti ti-layout-grid me-1"></i>{{ $sr->departement->departement ?? 'N/A' }}
                                </div>
                                <div class="text-muted small">
                                    <i class="ti ti-user me-1"></i>{{ $sr->user->name ?? '-' }}
                                </div>
                            </td>
                            <td>
                                <div class="fw-semibold mb-1">{{ $sr->vendor->vendor ?? '-' }}</div>
                                @php
                            ...
                            <td class="text-center">
                                <div class="small mb-1" title="Payment Due Date">
                                    ...
                                    {{ $sr->payment_due_date ? $sr->payment_due_date->format('d M Y') : '-' }}
                                </div>
                                <div class="small" title="Est. Settlement Date">
                                    <i class="ti ti-calendar-event text-info me-1"></i>
                                    {{ $sr->est_settlement_date ? $sr->est_settlement_date->format('d M Y') : '-' }}
                                </div>
                            </td>
                            <td class="text-center">
                                @php
                            ...
                            <td class="text-center">
                                <div class="d-flex justify-content-center gap-2">
                                    <a href="{{ route('settlement-reports.show', hashid_encode($sr->id_pr, 'pr')) }}" class="btn btn-icon bg-light-primary rounded-circle" title="View">
                                        <i class="ti ti-eye fs-5"></i>
                                    </a>

                                    @if($sr->status == 0 && (auth()->user()->level === 1 || auth()->user()->hasPermission('sr.delete') || auth()->user()->id_user == $sr->id_user))
                                    <a href="{{ route('settlement-reports.edit', hashid_encode($sr->id_pr, 'pr')) }}" class="btn btn-icon bg-light-warning rounded-circle" title="Edit">
                                    ...
                                    <button type="button"
                                        onclick="showConfirm({
                                                ...
                                                type: 'danger',
                                                onConfirm: () => @this.delete({{ $sr->id_pr }})
                                            })"
