<div class="payment-show user-show">
            ...
            <div class="card edit-card">
                ...
                <div class="card-header d-flex justify-content-between align-items-center flex-wrap gap-2">
                    ...
                    <div class="d-flex flex-wrap gap-2 align-items-center">
                        ...
                        @if($canPrint)
                        <a href="{{ route('settlement-reports.payment.print', $lastPayHash) }}" target="_blank" class="btn btn-light btn-sm text-primary fw-bold">
                            <i class="ti ti-printer"></i> Print
                        </a>
                        @endif
                        @if($canDownload)
                        <a href="{{ route('settlement-reports.payment.download', $lastPayHash) }}" target="_blank" class="btn btn-light btn-sm text-primary fw-bold">
                            <i class="ti ti-download"></i> Download
                        </a>
                        @endif
                        @endif

                        <a href="{{ route('settlement-reports.show', $srHash) }}" class="btn btn-outline-light btn-sm text-white border-white">
                ...
                <div class="card-body p-4 p-md-5">
                    ...
                    <div class="user-profile-header mb-4">
                        <div class="position-relative z-1">
                            ...
                            <h2 class="mb-1 fw-bold text-white">{{ $sr->pr->pr_number }}</h2>
                            <p class="mb-4 small opacity-75">{{ $sr->subject }} &bull; {{ $sr->vendor->vendor ?? '-' }}</p>
                            ...
                            <div class="row g-3 justify-content-center">
                                ...
                                <div class="col-md-3 col-6">
                                    <div class="glass-stat-card">
                                        ...
                                        <div class="amount-value text-white">{{ $symbol }} {{ number_format($this->receipt, 2, ',', '.') }}</div>
                                    </div>
                                </div>
                                <div class="col-md-3 col-6">
                                    <div class="glass-stat-card">
                                ...
                                <div class="col-md-3 col-6">
                                    <div class="glass-stat-card">
                                        ...
                                        <div class="amount-value {{ $balance != 0 ? 'text-warning' : 'text-success' }}">
                                            {{ $symbol }} {{ number_format($balance, 2, ',', '.') }}
                                        </div>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div class="form-section">
                        ...
                        <div class="row g-3">
                            <div class="col-md-4">
                                ...
                                <input type="text" class="form-control bg-light" value="{{ $sr->no_invoice ?: ($sr->pr->no_invoice ?? '-') }}" readonly>
                            </div>
                            <div class="col-md-4">
                                <label class="form-label small">Departemen / Cabang</label>
                                <input type="text" class="form-control bg-light" value="{{ $sr->departement->departement ?? '-' }} / {{ $sr->branch->branch ?? '-' }}" readonly>
                            </div>
                            <div class="col-md-4">
                                <label class="form-label small">Metode & Tipe Payment</label>
                                <input type="text" class="form-control bg-light" value="{{ $sr->payment_method == 1 ? 'Transfer' : 'Cash' }} — {{ $sr->payment_type_pr == 1 ? 'Butuh Approval' : 'Langsung' }}" readonly>
                            </div>
                            <div class="col-md-4">
                                <label class="form-label small">Bank Default</label>
                                <input type="text" class="form-control bg-light" value="{{ $sr->nama_bank ?: ($sr->norek_vendor->nama_bank ?? ($sr->pr->nama_bank ?: ($sr->pr->norek_vendor->nama_bank ?? '-'))) }}" readonly>
                            </div>
                            <div class="col-md-4">
                                <label class="form-label small">Atas Nama Default</label>
                                <input type="text" class="form-control bg-light" value="{{ $sr->nama_penerima ?: ($sr->norek_vendor->nama_penerima ?? ($sr->pr->nama_penerima ?: ($sr->pr->norek_vendor->nama_penerima ?? '-'))) }}" readonly>
                            </div>
                            <div class="col-md-4">
                                <label class="form-label small">No. Rekening Default</label>
                                <input type="text" class="form-control bg-light" value="{{ $sr->norek ?: ($sr->norek_vendor->norek ?? ($sr->pr->norek ?: ($sr->pr->norek_vendor->norek ?? '-'))) }}" readonly>
                            </div>
                        </div>
                    </div>
                            ...
                            <div class="card-header d-flex justify-content-between align-items-start flex-wrap gap-2 bg-light">
                                ...
                                <div class="d-flex gap-2 flex-wrap">
                                    ...
                                    <button type="button" class="btn btn-sm btn-danger" title="Delete Payment"
                                        onclick="window.showConfirm({title: 'Hapus Payment', message: 'Hapus payment ini?', type: 'danger', btnText: 'Ya, Lanjutkan!', onConfirm: () => window.location.href='{{ route('settlement-reports.payment.delete', $payHash) }}'})">
                            ...
                            <div class="card-body pb-2">
                                <div class="row g-3">
                                    <div class="col-md-3">
                                        ...
                                        <div class="fw-bold">{{ $payment->nama_bank ?: ($sr->nama_bank ?: ($sr->norek_vendor->nama_bank ?? ($sr->pr->nama_bank ?: ($sr->pr->norek_vendor->nama_bank ?? '-')))) }}</div>
                                    </div>
                                    <div class="col-md-3">
                                        <label class="form-label small">Atas Nama</label>
                                        <div class="fw-bold">{{ $payment->nama_penerima ?: ($sr->nama_penerima ?: ($sr->norek_vendor->nama_penerima ?? ($sr->pr->nama_penerima ?: ($sr->pr->norek_vendor->nama_penerima ?? '-')))) }}</div>
                                    </div>
                                    <div class="col-md-3">
                                        <label class="form-label small">No. Rekening</label>
                                        <div class="fw-bold">{{ $payment->norek ?: ($sr->norek ?: ($sr->norek_vendor->norek ?? ($sr->pr->norek ?: ($sr->pr->norek_vendor->norek ?? '-')))) }}</div>
                                    </div>
                                    <div class="col-md-3">
                                            ...
                                            <div class="d-flex flex-column align-items-center gap-1">
                                                ...
                                                <div class="d-flex gap-1">
                                                    ...
                                                    <button type="button" class="btn btn-xs btn-danger p-1" style="font-size:0.65rem;"
                                                        onclick="window.showConfirm({title: 'Hapus Attachment', message: 'Hapus attachment ini?', type: 'danger', btnText: 'Ya, Lanjutkan!', onConfirm: () => window.location.href='{{ route('settlement-reports.attachment-payment.delete', $att->id_attachment_payment) }}'})"
                                                        title="Delete">
                                                        <i class="ti ti-x"></i>
                                                    </button>
                                                </div>
    ...
    <div class="modal fade" id="modalTambahPayment" tabindex="-1">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <form action="{{ route('settlement-reports.payment.store', $srHash) }}" method="POST">
                    @csrf
                    ...
                    <div class="modal-body">
                        <div class="row g-3">
                            ...
                            <div class="col-md-6">
                                ...
                                <input type="text" name="nama_bank" class="form-control" value="{{ $sr->nama_bank ?: ($sr->norek_vendor->nama_bank ?? '') }}">
                            </div>
                            <div class="col-md-6">
                                <label class="form-label fw-bold">Nama Penerima</label>
                                <input type="text" name="nama_penerima" class="form-control" value="{{ $sr->nama_penerima ?: ($sr->norek_vendor->nama_penerima ?? '') }}">
                            </div>
                            <div class="col-md-6">
                                <label class="form-label fw-bold">No Rekening</label>
                                <input type="text" name="norek" class="form-control" value="{{ $sr->norek ?: ($sr->norek_vendor->norek ?? '') }}">
                            </div>
                            <div class="col-md-6">
    ...
    <div class="modal fade" id="modalEditPayment{{ $payment->id_payment }}" tabindex="-1">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <form action="{{ route('settlement-reports.payment.update', $payHash) }}" method="POST">
                    @csrf
                    ...
                    <div class="modal-body">
                        <div class="row g-3">
                            ...
                            <div class="col-md-6">
                                <label class="form-label fw-bold">Nama Bank</label>
                                <input type="text" name="nama_bank" class="form-control" value="{{ $payment->nama_bank }}">
                            </div>
                            <div class="col-md-6">
                                <label class="form-label fw-bold">Nama Penerima</label>
                            ...
                            <div class="col-md-6">
                                <label class="form-label fw-bold">No Rekening</label>
                                <input type="text" name="norek" class="form-control" value="{{ $payment->norek }}">
                            </div>
                            <div class="col-md-6">
    ...
    <div class="modal fade" id="modalAddAtt{{ $payment->id_payment }}" tabindex="-1">
        <div class="modal-dialog">
            <div class="modal-content">
                <form action="{{ route('settlement-reports.attachment-payment.store', $srHash) }}" method="POST" enctype="multipart/form-data" class="formPayAtt" id="formAddAttPay-{{ $payment->id_payment }}">
                    @csrf
    ...
    <div class="modal fade" id="modalEditAtt{{ $att->id_attachment_payment }}" tabindex="-1">
        <div class="modal-dialog">
            <div class="modal-content">
                <form action="{{ route('settlement-reports.attachment-payment.update', $att->id_attachment_payment) }}" method="POST" enctype="multipart/form-data" class="formPayAtt" id="formEditAttPay-{{ $att->id_attachment_payment }}">
                    @csrf
                    <input type="hidden" name="id_payment" value="{{ $payment->id_payment }}">
                    <div class="modal-header bg-warning">
