            ...
            <ul class="list-unstyled mb-0 d-flex align-items-center">
                ...
                <li class="dropdown pc-h-item header-user-profile">
                    <a
                        ...
                        aria-expanded="false">
                        @if(auth()->user()->photo)
                        <img src="{{ asset('storage/image/' . auth()->user()->photo) }}" alt="user-image" class="user-avtar">
                        @else
                        <img src="{{ asset('assets/images/user/avatar-2.jpg') }}" alt="user-image" class="user-avtar">
                        @endif
                        <span>{{ auth()->user()->position->position ?? 'User' }}</span>
                    </a>
                        ...
                        <div class="dropdown-header">
                            <div class="d-flex mb-1">
                                <div class="flex-shrink-0">
                                    @if(auth()->user()->photo)
                                    <img src="{{ asset('storage/image/' . auth()->user()->photo) }}" alt="user-image" class="user-avtar wid-35">
                                    @else
                                    <img src="{{ asset('assets/images/user/avatar-2.jpg') }}" alt="user-image" class="user-avtar wid-35">
                                    @endif
                                </div>
                                <div class="flex-grow-1 ms-3">
                                    <h6 class="mb-1 text-wrap">{{ auth()->user()->name }}</h6>
                                    <span>{{ auth()->user()->position->position ?? 'User' }}</span>
                                </div>
