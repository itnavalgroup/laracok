...
<div class="container">
    <div class="header flex-row">
        <div style="float: left; width: 60%">
            ...
            <div class="header-info" style="float: left; padding-top: 5px;">
                <h2 class="company-name">{{ $pr->company->company ?? 'SISTEM ERP' }}</h2>
                <p class="company-address">{{ $pr->company->address ?? '' }}</p>
                <p class="company-address">Phone: {{ $pr->company->phone ?? '-' }} | Email: {{ $pr->company->email ?? '-' }}</p>
            </div>
        ...
        <div class="col-left flex-col-left">
            <div class="info-box">
                ...
                <table class="info-table">
                    <tr>
                        <th>Vendor</th>
                        <td>: {{ $pr->vendor->vendor ?? '-' }}</td>
                    </tr>
                    <tr>
                        <th>Bank Name</th>
                        <td>: {{ $payment->nama_bank ?: ($pr->norek_vendor->nama_bank ?? '-') }}</td>
                    </tr>
                    <tr>
                        <th>Account Name</th>
                        <td>: {{ $payment->nama_penerima ?: ($pr->norek_vendor->nama_penerima ?? '-') }}</td>
                    </tr>
                    <tr>
                        <th>Account No</th>
                        <td>: {{ $payment->norek ?: ($pr->norek_vendor->norek ?? '-') }}</td>
                    </tr>
                </table>
            </div>
        </div>
    ...
    <div class="desc-box">
        ...
        <div class="desc-body">
            {{ $payment->payment_description ?: ($pr->subject . ' - Payment #' . $payment->id_payment) }}
            @if($payment->reason)
                <br><br>
        ...
        <div class="signatures flex-col-right" style="width: 48%; text-align: right;">
            @if($pr->payment_type_pr == 1)
                ...
                <div class="sign-box" style="display: inline-block; vertical-align: top; margin-right: 15px;">
                    ...
                    @if(str_contains($payment->filename ?? '', 'approved_manager') || str_contains($payment->filename ?? '', 'approved_director'))
                        <img src="data:image/svg+xml;base64,{{ base64_encode(generateQrSvg('Approved by Manager: ' . $pr->pr_number . ' - ' . \Carbon\Carbon::parse($payment->updated_at)->format('Y-m-d H:i'), 80, 0)) }}" class="qr-img" alt="QR Sign">
            ...
            <div class="sign-box" style="display: inline-block; vertical-align: top;">
                ...
                @if(str_contains($payment->filename ?? '', 'approved_director'))
                    <img src="data:image/svg+xml;base64,{{ base64_encode(generateQrSvg('Approved by Director: ' . $pr->pr_number . ' - ' . \Carbon\Carbon::parse($payment->updated_at)->format('Y-m-d H:i'), 80, 0)) }}" class="qr-img" alt="QR Sign">
                    <div class="sign-name">APPROVED</div>
                    <div class="sign-date">{{ \Carbon\Carbon::parse($payment->updated_at)->format('d M Y H:i') }}</div>
                @elseif($payment->filename === 'approved_by_system')
                    <img src="data:image/svg+xml;base64,{{ base64_encode(generateQrSvg('Auto Approved (Full Payment): ' . $pr->pr_number, 80, 0)) }}" class="qr-img" alt="QR">
