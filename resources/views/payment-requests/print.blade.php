...
<head>
    ...
    <title>Print PR - {{ $pr->pr_number }}</title>
    <style>
...
<div class="container">
    ...
    <div class="no-print" style="padding: 10px 0;">
        ...
        <a href="{{ route('payment-requests.show', hashid_encode($pr->id_pr, 'pr')) }}" class="btn-back">← Kembali</a>
    </div>
    ...
    <div class="doc-header">
        <div class="doc-title">
            ...
            <p>{{ $pr->company->company ?? '-' }} &bull; {{ $pr->docType->doc_type ?? '-' }}</p>
        </div>
        <div class="doc-meta">
        ...
        <div class="qr-section no-print">
            ...
            @endphp
            <img src="{{ $qrUrl }}" alt="QR Code" title="{{ $pr->pr_number }}">
        </div>
    </div>
    ...
    <table class="info-table">
        <tr><td class="label">Subject</td><td class="colon">:</td><td class="value">{{ $pr->subject ?? '-' }}</td>
            <td class="label">No Invoice</td><td class="colon">:</td><td class="value">{{ $pr->no_invoice ?? '-' }}</td></tr>
        <tr><td class="label">Tanggal</td><td class="colon">:</td><td class="value">{{ $pr->created_at?->format('d M Y') }}</td>
            <td class="label">PO Number</td><td class="colon">:</td><td class="value">{{ $pr->po_number ?? '-' }}</td></tr>
        <tr><td class="label">Departemen</td><td class="colon">:</td><td class="value">{{ $pr->departement->departement ?? '-' }}</td>
            <td class="label">Payment Method</td><td class="colon">:</td><td class="value">{{ $pr->payment_method == 1 ? 'Transfer' : 'Cash' }}</td></tr>
        <tr><td class="label">Cost Category</td><td class="colon">:</td><td class="value">{{ $pr->costCategory->cost_category ?? '-' }}</td>
            <td class="label">Payment Due Date</td><td class="colon">:</td><td class="value">{{ $pr->payment_due_date?->format('d M Y') ?? '-' }}</td></tr>
        <tr><td class="label">Cost Type</td><td class="colon">:</td><td class="value">{{ $pr->costType->cost_type ?? '-' }}</td>
            <td class="label">Est. Settlement</td><td class="colon">:</td><td class="value">{{ $pr->est_settlement_date?->format('d M Y') ?? '-' }}</td></tr>
        <tr><td class="label">Currency</td><td class="colon">:</td><td class="value">{{ $pr->currency ? $pr->currency->currency . ' (' . $pr->currency->symbol . ')' : 'IDR (Rp)' }}</td>
            <td class="label">Branch</td><td class="colon">:</td><td class="value">{{ $pr->branch->branch ?? '-' }}</td></tr>
        @if($pr->id_loan)
        <tr><td class="label">Loan</td><td class="colon">:</td><td class="value" colspan="3">{{ $pr->loan->loan ?? '-' }}</td></tr>
        @endif
    </table>
    ...
    <table class="info-table">
        <tr><td class="label">Vendor</td><td class="colon">:</td><td class="value">{{ $pr->vendor->vendor ?? '-' }}</td>
            <td class="label">Email Vendor</td><td class="colon">:</td><td class="value">{{ $pr->emailVendor->email ?? '-' }}</td></tr>
        <tr><td class="label">NPWP / NIK</td><td class="colon">:</td><td class="value">{{ $pr->vendor->npwp ?? '-' }}</td>
            <td class="label">Email User</td><td class="colon">:</td><td class="value">{{ $pr->emailUser->email ?? '-' }}</td></tr>
        @php
        ...
        @endphp
        <tr><td class="label">Bank</td><td class="colon">:</td><td class="value">{{ $bankName }}</td>
            <td class="label">Atas Nama</td><td class="colon">:</td><td class="value">{{ $bankHolder }}</td></tr>
        <tr><td class="label">No. Rekening</td><td class="colon">:</td><td class="value" colspan="3">{{ $bankNorek }}</td></tr>
    </table>

    <!-- ===== ITEM TABLE ===== -->
    <div class="section-title">Detail Item PR</div>
    @php $symbol = $pr->currency?->symbol ?? 'Rp'; @endphp
    <table class="item-table">
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{ $d->detail }}</td>
                <td class="center">{{ $d->bl_number ?? '-' }}</td>
                <td class="num">{{ number_format($d->qty, 2, ',', '.') }}</td>
                <td class="center">{{ $d->uom?->uom ?? '-' }}</td>
                <td class="num">{{ $symbol }} {{ number_format($d->price, 2, ',', '.') }}</td>
                <td class="num">{{ $symbol }} {{ number_format($d->discount ?? 0, 2, ',', '.') }}</td>
                <td class="num">{{ $symbol }} {{ number_format($d->tax1 ?? 0, 2, ',', '.') }}</td>
                <td class="num">{{ $symbol }} {{ number_format($d->tax2 ?? 0, 2, ',', '.') }}</td>
                <td class="num"><strong>{{ $symbol }} {{ number_format($d->ammount ?? 0, 2, ',', '.') }}</strong></td>
            </tr>
            @endforeach
        </tbody>
        <tfoot>
            @if(floatval($pr->additional_discount ?? 0) > 0)
            <tr>
                <td colspan="9" style="text-align:right">Additional Discount</td>
                <td class="num">{{ $symbol }} {{ number_format($pr->additional_discount, 2, ',', '.') }}</td>
            </tr>
            @endif
            <tr>
                <td colspan="9" style="text-align:right">Grand Total</td>
                <td class="num">{{ $symbol }} {{ number_format($grandTotal, 2, ',', '.') }}</td>
            </tr>
            <tr>
                <td colspan="9" style="text-align:right">Total Payment</td>
                <td class="num">{{ $symbol }} {{ number_format($paidTotal, 2, ',', '.') }}</td>
            </tr>
            <tr>
                <td colspan="9" style="text-align:right">Saldo</td>
                <td class="num">{{ $symbol }} {{ number_format($balance, 2, ',', '.') }}</td>
            </tr>
        </tfoot>
    </table>

    <!-- ===== SUPPORTING DOCUMENTS ===== -->
    @if($pr->attachmentPrs->count() > 0)
    <div class="section-title">Dokumen Pendukung</div>
    <ul class="attachment-list">
        @foreach($pr->attachmentPrs as $att)
        <li>
            <span class="check-icon">✓</span>
            {{ $att->attachment?->attachment ?? 'Dokumen' }}
            @if($att->notes) &mdash; <em>{{ $att->notes }}</em> @endif
        </li>
        @endforeach
    </ul>
    @endif
    ...
    <div class="sign-section">
        ...
        <div class="sign-grid sign-grid-4" style="margin-top:12px;">
            @foreach($signSteps as $step => $role)
            ...
            <div class="sign-box">
                ...
                <div class="sign-area">
                    @if($st)
                        ...
                        <div class="sign-date">{{ \Carbon\Carbon::parse($st->created_at)->format('d/m/Y') }}</div>
    ...
    <div class="sign-section" style="margin-top:20px;">
        ...
        <div class="sign-grid sign-grid-3" style="margin-top:12px;">
            @foreach($signSteps2 as $step => $role)
            @php $st = $signMap[$step] ?? null; @endphp
            <div class="sign-box">
                <div class="sign-role">{{ $role }}</div>
                <div class="sign-area">
    ...
    <div class="print-footer">
        Dokumen ini dicetak dari Sistem PPD Naval &bull; {{ now()->format('d M Y H:i') }}
    </div>
</div>
</body>
</html>
